...
$this.load = async function(){
	...
	// Load pairs
	{
		...
		for (const pair_data of dm_pairs.pairs){
			$this.create_chaotic_pair(pair_data)
		}

		// Set active pair, if any
		if (mctx.cache.active_pair >= 0){
			const tgt_pair = $this.chaotic_pairs.at(mctx.cache.active_pair);
			if (tgt_pair){
				$this.select_pair(tgt_pair);
			}
		}
...
$this.ChaoticPlayerInstance = class {
...
$this.ChaoticPair = class {
	constructor(_input_data=null){
		...
		this.ctrl_base_dom.elem.oncontextmenu = function(evt){
			if (evt.altKey && $this.edit_mode_active){
				// self kill self
				// javascript moment...
				self.kill(self)
				return
		...
		for (const side of ['left', 'mid', 'right',]){
			...
			const player_instance = new $this.ChaoticPlayerInstance(
				input_data[side],
				function(){
	...
	kill(self){
		...
		$this.chaotic_pairs.delete(self);
...
$this.create_chaotic_pair = function(pair_data=null){
...
$this.save_player_base = function(){
	...
	const data = {
		...
		// 'active_pair': $this.chaotic_pairs.indexof($this.active_pair),
		'pairs': [],
	}
...
$this.toggle_edit_mode = function(){
	$this.edit_mode_active = !$this.edit_mode_active;
	$('[tabid="ctrl_base"]').attr('edit_mode', $this.edit_mode_active);

}

$this.push_scores_to_vmix = async function(){
	print('Pushing scores to vmix');
	$this.titles.fight_tracker.set_text(
		'fight_score_l',
		$this.current_pair.players.left.score
	)
	$this.titles.fight_tracker.set_text(
		'fight_score_r',
...
$this.push_current_players_to_vmix = async function(){
	print('Pushing players to vmix');
	$this.titles.fight_tracker.set_text(
		'player_name_l',
		(
			ksys.strf.params.player_names.format($this.current_pair.players.left.data_inputs.player_name.value)
			+
			' '
			+
			ksys.strf.params.player_names.format($this.current_pair.players.left.data_inputs.player_surname.value)
		)
	)
	$this.titles.fight_tracker.set_text(
		'player_name_r',
		(
			ksys.strf.params.player_names.format($this.current_pair.players.right.data_inputs.player_name.value)
			+
			' '
			+
			ksys.strf.params.player_names.format($this.current_pair.players.right.data_inputs.player_surname.value)
		)
	)
}
...
$this.select_pair = function(tgt_pair=null, upd_vmix=true){
	...
	ksys.context.module.prm('active_pair', $this.chaotic_pairs.indexof($this.current_pair))

	// Create control classes
	const player_left = new $this.ChaoticPlayerFightControl(tgt_pair.players.left);
	const player_right = new $this.ChaoticPlayerFightControl(tgt_pair.players.right);
...
$this.fight_tracker_vis_ctrl = function(state){
	if (state == true){
		$this.titles.fight_tracker.overlay_in(1);
	}
	if (state == false){
		$this.titles.fight_tracker.overlay_out(1);
	}
}
