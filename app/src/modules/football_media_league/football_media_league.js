...
$this.load = async function(){
	...
	// --------------------------
	{
		...
		$this.final_scores_bg_fpath = ksys.context.module.prm('final_scores_bg_fpath') || null;
	}
...
$this.apply_resources = async function(){
	const resDir = Path(
		$this.resources_location.value || ''
	)
	...
	for (const fpath of flist){
		...
		if (!isKnown){
			...
			$this.final_scores_bg_fpath = fpath;
			continue
		}
...
$this.add_score_from_cards_panel = async function(auto=false){
	...
	const tgt_lineup = $this.resource_index.side[side].lineup;
