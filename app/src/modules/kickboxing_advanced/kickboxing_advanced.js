...
$this.KBPlayerPair = class{
	...
	prepare_winners(self){
		...
		red_btn.onclick = function(){
			$this.show_winner(self.players.red);
		}

		blu_btn.onclick = function(){
			$this.show_winner(self.players.blu);
		}
	}
}
...
$this._update_vs_title = async function(tgt_pair){
	if ($this.edit_mode_active){return};
...
$this.main_event_vis = async function(){
	await $this.update_vs_title({
		'tgt_pair': $this.pair_list.get(
			[...qselAll('#player_list > .player_pair')].at(-1)
		),
		'update_timer': false,
		'tgt_title': $this.titles.main_event,
		'photo_src': 'pair_pool_rect',
	})

	ksys.btns.adv_timeout({
		'main_event_onn':          $this.titles.main_event.anim_durations[null],
		'comain_event_onn':        $this.titles.main_event.anim_durations[null],
		'main_w_comain_event_onn': $this.titles.main_event.anim_durations[null],
	})

	await $this.titles.main_event.overlay_in();
}

$this.comain_event_vis = async function(){
	await $this.update_vs_title({
		'tgt_pair': $this.pair_list.get(
			[...qselAll('#player_list > .player_pair')].at(-2)
		),
		'update_timer': false,
		'tgt_title': $this.titles.comain_event,
		'photo_src': 'pair_pool_rect',
	})

	ksys.btns.adv_timeout({
		'main_event_onn':          $this.titles.comain_event.anim_durations[null],
		'comain_event_onn':        $this.titles.comain_event.anim_durations[null],
		'main_w_comain_event_onn': $this.titles.comain_event.anim_durations[null],
	})

	await $this.titles.comain_event.overlay_in();
}

$this.main_w_comain_event_vis = async function(){
	await $this.update_vs_title({
		'tgt_pair': $this.pair_list.get(
			[...qselAll('#player_list > .player_pair')].at(-1)
		),
		'update_timer': false,
		'tgt_title': $this.titles.main_w_comain,
		'field_suffix': '_1',
		'photo_src': 'pair_pool_rect',
	})
	await $this.update_vs_title({
	...
	ksys.btns.adv_timeout({
		'main_event_onn':          $this.titles.main_w_comain.anim_durations[null],
		'comain_event_onn':        $this.titles.main_w_comain.anim_durations[null],
		'main_w_comain_event_onn': $this.titles.main_w_comain.anim_durations[null],
	})

	await $this.titles.main_w_comain.overlay_in();
}

$this.hide_events = async function(){
	ksys.btns.toggle({
	...
	await $this.titles.main_event.overlay_out_all();
	await $this.titles.comain_event.overlay_out_all();
	await $this.titles.main_w_comain.overlay_out_all();
...
$this.undercard_show = async function(offs=0){
	const per_page = 4;
	...
	for (const idx of range(per_page)){
		const tgt_pair = $this.pair_list.get(
		...
		await $this.update_vs_title({
			'tgt_pair': tgt_pair,
			'update_timer': false,
			'tgt_title': $this.titles.undercard,
	...
	ksys.btns.adv_timeout({
		'undercard_onn_1': $this.titles.undercard.anim_durations[null],
		'undercard_onn_2': $this.titles.undercard.anim_durations[null],
	})

	await $this.titles.undercard.overlay_in();
...
$this.undercard_hide = async function(){
	ksys.btns.toggle({
		'undercard_onn_1': false,
		'undercard_onn_2': false,
		'undercard_off':   false,
	...
	await $this.titles.undercard.overlay_out_all();
...
$this.show_judges = async function(){

	let judges = [];
	for (const player_data of Object.values($this.current_pair.players)){
	...
	for (const idx of range(3)){
		await $this.titles.judges.set_text(
			`name_${idx+1}`,
			(judges[idx] || '').upper()
		)
	}
	...
	ksys.btns.adv_timeout({
		'judges_onn':      $this.titles.judges.anim_durations[null],
		'referee_onn':     $this.titles.judges.anim_durations[null],
		'upcoming_center': $this.titles.judges.anim_durations[null],
		'upcoming_corner': $this.titles.judges.anim_durations[null],
	})

	await $this.titles.judges.overlay_in();
}

$this.show_referee = async function(){
	let referee = '';
	for (const player_data of Object.values($this.current_pair.players)){
		if (player_data.attr_list?.judges?.value){
	...
	ksys.btns.adv_timeout({
		'judges_onn':      $this.titles.referee.anim_durations[null],
		'referee_onn':     $this.titles.referee.anim_durations[null],
		'upcoming_center': $this.titles.referee.anim_durations[null],
		'upcoming_corner': $this.titles.referee.anim_durations[null],
	})

	await $this.titles.referee.overlay_in();
}

$this.upcoming_show = async function(variant){
	const title = (variant == 'center') ? $this.titles.upcoming_center : $this.titles.upcoming_corner;

	await $this.update_vs_title({
		'tgt_pair': $this.pair_list.get(
			$this.current_pair.dom.root.nextSibling
		),
		'update_timer': false,
		'tgt_title': title,
		'photo_src': 'pair_pool_sq',
...
$this.judges_off = async function(){
	ksys.btns.toggle({
		'judges_onn':      false,
		'referee_onn':     false,
	...
	await $this.titles.judges.overlay_out_all();
	await $this.titles.referee.overlay_out_all();

	await $this.titles.upcoming_center.overlay_out_all();
	await $this.titles.upcoming_corner.overlay_out_all();
...
$this.show_winner = async function(tgt_player){
	const title = $this.titles.winner;
	const frmt = ksys.strf.params.players;
	...
	ksys.btns.adv_timeout({
		'winner_red': $this.titles.winner.anim_durations[null],
		'winner_blu': $this.titles.winner.anim_durations[null],
	})

	await title.overlay_in();
}

$this.hide_winner = async function(){
	ksys.btns.toggle({
		'winner_red': false,
		'winner_blu': false,
		'winner_off': false,
	...
	await $this.titles.winner.overlay_out_all();
