...
$this.load = async function(){
	...
	$this.main_clock_id = 'futsal_uefa.main';
	$this.extra_clock_id = 'futsal_uefa.extra';

	$this.red_clock_right_id = 'futsal_uefa.red.left';
	$this.red_clock_left_id = 'futsal_uefa.red.right';
	...
	// --------------------------
	{
		$this.field_layout_template_fpath = document.querySelector('#field_layout_template_fpath');
		$this.field_layout_template_fpath.value = mctx.cache.field_layout_template_fpath || '/';
	...
	{
		// Important todo: GTZ Shape is the only one that works
		$this.lineup_condensed_max_players_per_page = new ksys.switches.KBSwitch({
			'multichoice': false,
			'can_be_empty': false,
	...
	// --------------------------
	{
		$this.shootout_switch_0 = new ksys.switches.KBSwitch({
			'multichoice': false,
			'can_be_empty': true,
			// 'set_default': null,
			'dom_array': [
			...
			'callback': function(kbswitch, schema_id){
				$this.redraw_shootout();
			}
		});
		$this.shootout_switch_1 = new ksys.switches.KBSwitch({
			'multichoice': false,
			'can_be_empty': true,
			// 'set_default': null,
			'dom_array': [
	...
	// --------------------------
	{
		...
		if (tgt_timer_fset == 'at_at'){
			...
			// RED Countdown LEFT
			ksys.ticker.bundestag.attach({
				'clock_id': $this.red_clock_left_id,
				'tick':     $this.red_clock_left_tick_callback,
				'end':      $this.red_clock_left_end_callback,
			})

			// RED Countdown RIGHT
			ksys.ticker.bundestag.attach({
				'clock_id': $this.red_clock_right_id,
				'tick':     $this.red_clock_right_tick_callback,
				'end':      $this.red_clock_right_end_callback,
			})

			$this.timer_fset.builtin.timer_callback(
				await ksys.ticker.bundestag.read_clock($this.main_clock_id)
			)
			$this.timer_fset.builtin.extra_timer_callback(
				await ksys.ticker.bundestag.read_clock($this.extra_clock_id)
			)
		}
	...
	// --------------------------
	{
		...
		$this.misc_titles = Object.freeze({
			...
			'ball_possession': new $this.MiscTitle({
	...
	*/
	...
	for (const manualTable of qselAll('.manual_table_ctrl')){
		(new $this.ManualTable(manualTable)).load();
	}
}
...
$this.TeamLineup = class{
	constructor(club, colors=null, input_lineup_info=null){
		...
		self.fouls = new $this.TeamFouls(self);
...
$this.CardManager = class {
	constructor(init_data=null){
		...
		for (const side of ['home', 'guest']){
			...
			tplate.index.cancel_red.onclick = async function(){
				...
				await $this.eval_red_countdown_vis();
				await $this.cancel_red_countdown(side);
				self.cancel_red(side);
	...
	async _show_card_title(self, title, player){
		if ($this.title_schema_switch.selected == 'paged'){
			...
			const timeout_dur = ksys.util.sum([
				...
				$this.titles.timer.anim_durations['TransitionOut'],
				$this.titles.timer.anim_durations[null],
			])
			...
			await $this.titles.timer.overlay_out(null, false);
			...
			// Bring back timer
			await $this.titles.timer.overlay_in();
	...
	async show_card_title(self, title, player){
		if ($this.title_schema_switch.selected == 'paged'){
			...
			const [animPromise, seqTime] = $this.titles.timer.replace(
				title,
				$this.sequencer.sequences.cards.items.hold.dur
			);
	...
	async hand_yellow(self, player){
		...
		if (pcard_info.red){
			...
			await $this.begin_red_countdown(pside);
		}
...
$this.ClubGoals = class {
		...
		locked=false)
	{
		...
		// const offs = int(ksys.context.module.prm('round_num')) * $this.round_duration;
		// const calculated_timestamp = offs - (timestamp_override || $this.get_current_time(true));
...
$this.TeamFouls = class{
	...
	async redrawVMIX(self){
		await $this.titles.timer.set_img_src(
			...
			Path($this.resources_location.value, 'fouls', `${self.foulsAmount}.png`)
		)
	}
}

$this.ManualTable = class{
	...
	constructor(rootDOM){
		...
		self.targetTitle = $this.titles[self.tableID];
	...
	async resyncVMIX(self){
		...
		if (self.includeStats){
			await $this.apply_stats_header(self.targetTitle);
		}
...
$this._forward_field_layout_to_vmix = async function(team){
	...
	const title = $this.titles[`team_layout_${suffix}`];
	...
	const template_gtz = Path($this.field_layout_template_fpath.value);
	...
	if ($this.cmd_layout_reserves_schema_switch.selected == 'dedicated'){
		const data_mapping = [
			// {
			// 	'plist': tgt_field.lineup.main_players,
			// 	'slot_type': 'm',
...
$this.show_field_layout_condensed = async function(team){
	const max_per_page = int($this.lineup_condensed_max_players_per_page.selected);
	const tgt_side = str(team).lower();
	const tgt_lineup = $this.resource_index.side?.[tgt_side]?.lineup;
	const role_gropus = {};
...
$this.redraw_shootout = async function(){
	...
	await $this.titles.shootout.set_text(
		'score_sum',
	...
	await $this.titles.shootout.set_img_src(
		'logo_l',
		$this.resource_index.side.home.club.logo_path
	)
	await $this.titles.shootout.set_img_src(
	...
	for (const i of range(2)){
		const state = $this[`shootout_switch_${i}`].selected;

		// let tgt_icon = Path($this.resources_location.value, 'icons/p_no.png');
		let tgt_icon = Path($this.resources_location.value, 'icons/p_neutral.png');;
...
$this.show_shootout_title = async function(){
	await $this.redraw_shootout();
	await $this.titles.shootout.overlay_in();
}


$this.hide_shootout_title = async function(){
	await $this.titles.shootout.overlay_out();
}
...
$this.show_misc_title = async function(tgt_title){
	if (tgt_title == 'commenter'){
		const names = [];
		for (const row of $this.misc_titles.commenter.fields.commenter_names.value.split('\n')){
		...
		const tgt_title = $this.titles[`commenter_x${names.length}`];
	...
	if (tgt_title == 'ball_possession'){
		// $this.misc_titles.commenter.fields.commenter_names.value

		await $this.titles.ball_possession.set_img_src(
			'logo_l',
			$this.resource_index.side?.home?.club?.logo_path
		)
		...
		await $this.titles.ball_possession.set_text(
			'text_data',
			$this.misc_titles.ball_possession.fields.ball_possession.value
		)

		const [animPromise, animDuration] = $this.titles.timer.replace(
			$this.titles.ball_possession
		)
...
$this.show_coach = async function(side){
	...
	$this.coach_timer = (
		!!(await $this.titles.timer.list_occupied_overlays()).length
	);
	...
	if ($this.coach_timer){
		[animPromise, seqTime] = $this.titles.timer.replace(
			$this.titles.coach
		);
	}else{
		[animPromise, seqTime] = [
			$this.titles.coach.overlay_in(),
			$this.titles.coach.anim_durations[null]
		]
	}
...
$this.hide_coach = async function(){
	...
	if ($this.coach_timer){
		[animPromise, seqTime] = $this.titles.coach.replace(
			$this.titles.timer
		);
	}else{
		[animPromise, seqTime] = [
			$this.titles.coach.overlay_out(),
			$this.titles.coach.anim_durations['TransitionOut']
		]
	}
...
$this.begin_red_countdown = async function(side){
	let clockID = null;
	let clockFieldID = null;

	if (side == 'home'){
		clockID = $this.red_clock_left_id;
		clockFieldID = 'red.clock.l';
	}else{
		clockID = $this.red_clock_right_id;
...
$this.cancel_red_countdown = async function(side){
	let clockID = null;
	let selector = null;
...
$this.red_clock_left_tick_callback = function(data){
...
$this.red_clock_right_tick_callback = function(data){
	if (!data?.time?.total){return};
	const time = data.time;
...
$this.red_clock_left_end_callback = async function(){
	qsel(`#red_feedback_left`).textContent = '--:--';
	ksys.context.module.prm('red_countdown_home', false);
	await $this.eval_red_countdown_vis();
}
$this.red_clock_right_end_callback = async function(){
	qsel(`#red_feedback_left`).textContent = '--:--';
...
$this.toggle_config_vis = function(){
...
$this.apply_preset = function(preset_id, evt=null){
	if (evt && !evt.altKey){
		ksys.info_msg.send_msg(
			`Hold ALT`,
			'warn',
...
$this.timeout_vis = async function(side){
	if (side == false){
		ksys.btns.adv_timeout({
			'show_timeout_home':  $this.titles.timeout.anim_durations['TransitionOut'],
			'show_timeout_guest': $this.titles.timeout.anim_durations['TransitionOut'],
			'hide_timeout_title': $this.titles.timeout.anim_durations['TransitionOut'],
		})
		await $this.titles.timeout.overlay_out();
		return
	}

	$this.titles.timeout.set_img_src(
		'club_logo',
		$this.resource_index.side?.[side]?.field?.lineup?.club?.logo_path
	)

	ksys.btns.adv_timeout({
		'show_timeout_home':  $this.titles.timeout.anim_durations[null],
		'show_timeout_guest': $this.titles.timeout.anim_durations[null],
		'hide_timeout_title': $this.titles.timeout.anim_durations[null],
	})
	await $this.titles.timeout.overlay_in();
}
...
$this.get_current_time = function(minutes=false, tsum=false){
	...
	if (tsum){
		...
		return {
			'base': ksys.util.clamp((offs - base_t) + 1, 1, $this.round_duration * 2),
			'extra': Math.ceil(
				(extra_t || 0) / divider
			),
		}
...
$this.main_timer_vis = async function(state){
	...
	if (state == true){
		...
		// Redraw fouls
		await $this.resource_index.side.guest?.lineup?.fouls?.redrawVMIX?.();
		await $this.resource_index.side.home?.lineup?.fouls?.redrawVMIX?.();
...
$this.extra_time_vis = async function(state){
	...
	if (state == false){
		...
		if ($this.title_schema_switch.selected == 'paged'){
			await $this.titles.timer.set_shape_color('main_time_bg', 'D80D83FF');
		}
...
$this.timer_fset.builtin.start_base_timer = async function(rnum){
	...
	// Reset fouls
	$this.resource_index?.home_lineup?.fouls?.setFouls?.(0);
	$this.resource_index?.guest_lineup?.fouls?.setFouls?.(0);
	await $this.resource_index?.home_lineup?.fouls?.redrawVMIX?.();
	await $this.resource_index?.guest_lineup?.fouls?.redrawVMIX?.();
...
$this.timer_fset.builtin.pause = async function(){
	await ksys.ticker.bundestag.edit_clock({
		'clock_id': $this.main_clock_id,
		'action': 'pause',
		'data': {}
...
$this.timer_fset.builtin.resume = async function(){
	await ksys.ticker.bundestag.edit_clock({
		'clock_id': $this.main_clock_id,
		'action': 'resume',
		'data': {}
...
$this._edit_main_clock_offset = function(offs){
...
$this.edit_main_clock_offset = function(offs){
...
$this.add_score_from_cards_panel = async function(auto=false){
...
$this.apply_stats_header = async function(tgt_title){
...
// todo: get rid of ?.
$this.___show_score_summary = async function(){
	await $this.update_team_colors();
	...
	// Set bottom lines text
	await $this.titles.final_scores.set_text('title_lower_top', $('#vs_text_bottom_upper').val())
	await $this.titles.final_scores.set_text('title_lower_bot', $('#vs_text_bottom_lower').val())

	// Set main header
	await $this.titles.final_scores.set_text('title_upper', $('#vs_text_main_header').val())
...
$this.show_score_summary_SINGLE = async function(extras='time'){
	await $this.update_team_colors();
	...
	// Misc.
	// ------------------------------
	await $this.apply_stats_header(
		$this.titles.final_scores
	)

	if (extras == 'clock'){
		$this.titles.final_scores.toggle_text('clock', true);
		$this.titles.final_scores.toggle_text('time', false);
	}else{
		$this.titles.final_scores.toggle_text('clock', false);
		$this.titles.final_scores.toggle_text('time', true);
	}

	ksys.btns.adv_timeout({
		'show_score_results_clock': $this.titles.final_scores.anim_durations[null],
		'show_score_results_time':  $this.titles.final_scores.anim_durations[null],
	})
...
$this.show_score_summary = async function(extras='time'){
	await $this.update_team_colors();
	...
	if (score_summary.home.length > 7 || score_summary.guest.length > 7){
		...
		for (const page of pages){
			...
			await ksys.util.sleep(
				$this.sequencer.sequences.final_scores.items.hold_page.dur
			)
		}
	}else{
		await $this.titles.final_scores.set_text(
...
$this.hide_score_summary = async function(){
	const timeoutDur = $this.titles.final_scores.anim_durations['TransitionOut'];
	ksys.btns.adv_timeout({
...
$this.hide_penalty_title = async function(){
	const tgt_title = $this.titles.penalties;
	ksys.btns.adv_timeout({
		'hide_penalties': $this.titles.penalties.anim_durations['TransitionOut'],
		'show_penalties': $this.titles.penalties.anim_durations['TransitionOut'],
	})
