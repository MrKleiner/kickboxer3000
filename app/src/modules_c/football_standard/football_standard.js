	...
	async resync_vmix(self){
		...
		for (const _side_idx in retarded){
			...
			for (const card_idx of range(3)){
				await window.kbmodules.football_standard.titles.timer.toggle_img(`rcard_${side_idx+1}_${card_idx+1}`, !!side.red_stack.at(card_idx))
			}
		}
	}
...
window.kbmodules.football_standard._forward_field_layout_to_vmix = async function(team){
