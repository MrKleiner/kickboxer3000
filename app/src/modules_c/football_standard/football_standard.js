...
window.modules.football_standard.show_chosen_club_logo = function(team){
...
window.modules.football_standard.club_name_dynamic_type = function(event, team){
	const _team_selector = {
		1: {
			field: '#team1_layout',
			def: '#team1_def',
...
window.modules.football_standard.spawn_player = function(event){
		...
		if ( (fullname_code.includes(tquery) || number.includes(tquery)) ){
			punish_pool.append(`
				<div
					...
					surname="${surname}"
					team_logo="${club_logo}"
					onclick="window.modules.football_standard.select_player_for_punishment(event)"
				>
					<div class="player_number">${number}</div>
...
window.modules.football_standard.filter_players_replacement = function(event){
	const team_relation = {
		...
		if (name_code.includes(tquery) || number.includes(tquery)){
			replacement_pool.append(`
				<div
					...
					pname="${name}"
					surname="${surname}"
					onclick="window.modules.football_standard.mark_replacement_player(event)"
					team_logo="${club_logo}"
				>
					<div class="player_number">${number}</div>
...
window.modules.football_standard.replacement_player_title = async function(event){
	...
	await window.modules.football_standard.titles.replacement_out.set_text('player_name', leaving_player.attr('surname'))
	await window.modules.football_standard.titles.replacement_out.set_img_src('club_logo', leaving_player.attr('team_logo'))
	await window.modules.football_standard.titles.replacement_in.set_text('player_name', incoming_player.attr('surname'))
	await window.modules.football_standard.titles.replacement_in.set_img_src('club_logo', incoming_player.attr('team_logo'))
	...
	if (tgtbtn.getAttribute('btname') == 'subst_hide_leaving'){
		...
		await window.modules.football_standard.titles.replacement_out.overlay_out(1)
...
window.modules.football_standard.goal_score_on = async function(){
	...
	await window.modules.football_standard.titles.gscore.set_text('player_name', selected_player.getAttribute('surname').toUpperCase())
	await window.modules.football_standard.titles.gscore.set_img_src('club_logo', selected_player.getAttribute('team_logo'))
	// disable buttons
