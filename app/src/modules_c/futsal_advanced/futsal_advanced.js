...
window.kbmodules.futsal_advanced.timer_fset = {
	'at_at': {},
	'builtin': {},
...
window.kbmodules.futsal_advanced.ticker_time = {
	'base':{
		'minutes': 0,
		'seconds': 0,
	},
...
window.kbmodules.futsal_advanced.round_duration = 45;

window.kbmodules.futsal_advanced.load = async function(){
	const mctx = ksys.context.module;

	window.kbmodules.futsal_advanced.main_clock_id = 'fball.main';
	window.kbmodules.futsal_advanced.extra_clock_id = 'fball.extra';


	window.kbmodules.futsal_advanced.resource_index = {
		available_colors: [
			'0066c3',
			'00984c',
			'00aff1',
		...
		penalty_manager: new window.kbmodules.futsal_advanced.PenaltyManager(5),
	...
	window.kbmodules.futsal_advanced.time_lenghts_dict = {
		1: (0, 45),
		2: (45, 90),
		3: (90, 105),
		4: (105, 120),
	...
	// Round duration
	{
		...
		window.kbmodules.futsal_advanced.round_duration = mctx.cache.rdur || 45;
		dur_input.onchange = function(){
			const dur = int(dur_input.value.trim() || 45)
			window.kbmodules.futsal_advanced.round_duration = dur;
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.img_magick = document.querySelector('#image_magick_path');
		window.kbmodules.futsal_advanced.img_magick.value = mctx.cache.img_magick || '/';
		window.kbmodules.futsal_advanced.img_magick.onchange = function(){
			window.kbmodules.futsal_advanced.img_magick.value = window.kbmodules.futsal_advanced.img_magick.value.replaceAll('"', '');
			ksys.context.module.prm('img_magick', window.kbmodules.futsal_advanced.img_magick.value);
		}
	}
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.ffmpeg = document.querySelector('#ffmpeg_path');
		window.kbmodules.futsal_advanced.ffmpeg.value = mctx.cache.ffmpeg || '/';
		window.kbmodules.futsal_advanced.ffmpeg.onchange = function(){
			window.kbmodules.futsal_advanced.ffmpeg.value = window.kbmodules.futsal_advanced.ffmpeg.value.replaceAll('"', '');
			ksys.context.module.prm('ffmpeg', window.kbmodules.futsal_advanced.ffmpeg.value);
		}
	}
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.field_layout_template_fpath = document.querySelector('#field_layout_template_fpath');
		window.kbmodules.futsal_advanced.field_layout_template_fpath.value = mctx.cache.field_layout_template_fpath || '/';
		window.kbmodules.futsal_advanced.field_layout_template_fpath.onchange = function(){
			window.kbmodules.futsal_advanced.field_layout_template_fpath.value = window.kbmodules.futsal_advanced.field_layout_template_fpath.value.replaceAll('"', '');
			ksys.context.module.prm('field_layout_template_fpath', window.kbmodules.futsal_advanced.field_layout_template_fpath.value);
		}
	}
	...
	// --------------------------
	{
		...
		window.kbmodules.futsal_advanced.title_schema_switch = new ksys.switches.KBSwitch({
			'multichoice': false,
			'can_be_empty': false,
			...
			'callback': function(kbswitch, schema_id){
				...
				window.kbmodules.futsal_advanced.update_title_schema(schema_id);
			}
		});

		if (window.kbmodules.futsal_advanced.title_schema_switch.selected == 'paged'){
			window.kbmodules.futsal_advanced.persistent_header = new window.kbmodules.futsal_advanced.PersistentHeader();
			await window.kbmodules.futsal_advanced.persistent_header.pages.pull_page_from_vmix();
		}
	}
	...
	{
		// Important todo: GTZ Shape is the only one that works
		window.kbmodules.futsal_advanced.lineup_condensed_max_players_per_page = new ksys.switches.KBSwitch({
			'multichoice': false,
			'can_be_empty': false,
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.commenter_schema_switch = new ksys.switches.KBSwitch({
			'multichoice': false,
			'can_be_empty': false,
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.final_scores_color_schema_switch = new ksys.switches.KBSwitch({
			'multichoice': false,
			'can_be_empty': false,
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.timer_team_color_schema_switch = new ksys.switches.KBSwitch({
			'multichoice': false,
			'can_be_empty': false,
			'set_default': mctx.cache.final_scores_color_schema,
			'dom_array': [
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.cmd_layout_mains_schema_switch = new ksys.switches.KBSwitch({
			'multichoice': false,
			'can_be_empty': false,
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.timer_display_schema_switch = new ksys.switches.KBSwitch({
			'multichoice': false,
			'can_be_empty': false,
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.timer_main_clock_vis_schema_switch = new ksys.switches.KBSwitch({
			'multichoice': false,
			'can_be_empty': false,
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.cmd_layout_reserves_schema_switch = new ksys.switches.KBSwitch({
			'multichoice': false,
			'can_be_empty': false,
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.red_card_schema_switch = new ksys.switches.KBSwitch({
			'multichoice': false,
			'can_be_empty': false,
		...
		window.kbmodules.futsal_advanced.red_card_schema_color = document.querySelector('#red_card_schema_color');
		window.kbmodules.futsal_advanced.red_card_schema_color.value = mctx.cache.red_card_schema_color || 'ff0000';
		window.kbmodules.futsal_advanced.red_card_schema_color.onchange = function(){
			ksys.context.module.prm('red_card_schema_color', window.kbmodules.futsal_advanced.red_card_schema_color.value);
		}
	}
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.resources_location = document.querySelector('#resources_location');
		window.kbmodules.futsal_advanced.resources_location.value = mctx.cache.resources_location || '/';
		window.kbmodules.futsal_advanced.resources_location.onchange = function(){
			window.kbmodules.futsal_advanced.resources_location.value = window.kbmodules.futsal_advanced.resources_location.value.replaceAll('"', '');
			ksys.context.module.prm('resources_location', window.kbmodules.futsal_advanced.resources_location.value);
		}
	}
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.pname_schema_switch = new ksys.switches.KBSwitch({
			'multichoice': false,
			'can_be_empty': false,
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.role_editor = new window.kbmodules.futsal_advanced.PlayerRolesEditor();

		window.kbmodules.futsal_advanced.role_editor.load();

		document.querySelector('#role_editor').append(window.kbmodules.futsal_advanced.role_editor.editor_dom.root);
	}
	...
	// --------------------------
	{
		...
		window.kbmodules.futsal_advanced.resource_index.club_selector_dropdown = new window.kbmodules.futsal_advanced.ClubSelectorDropdown();

		// Card/goal player picker
		window.kbmodules.futsal_advanced.resource_index.card_player_filter = new window.kbmodules.futsal_advanced.PlayerPicker(
			[[]],
			function(){return true},
			function(list_dom, player_class){
				const card_manager = window.kbmodules.futsal_advanced.resource_index.card_manager;
		...
		window.kbmodules.futsal_advanced.resource_index.card_manager = new window.kbmodules.futsal_advanced.CardManager();
		// Todo: it's stupid this is here
		await window.kbmodules.futsal_advanced.show_red_cards();

		// Create substitutes
		for (const side of ['home', 'guest']){
			for (const rtype of ['leaving', 'inbound']){
				window.kbmodules.futsal_advanced.resource_index.side[side].substitute[rtype] = 
				new window.kbmodules.futsal_advanced.PlayerPicker(
					[[]],
					function(){return true},
					function(list_dom){
						list_dom.onclick = function(){
							const opposite_side = (side == 'home') ? 'guest' : 'home';
							window.kbmodules.futsal_advanced.resource_index.side[opposite_side].substitute.leaving.reset_selection()
							window.kbmodules.futsal_advanced.resource_index.side[opposite_side].substitute.inbound.reset_selection()
						}
					}
				)

				$(`#replacement_teams .replacement_team[${side}] .replacement_list[${rtype}]`)
				.append(window.kbmodules.futsal_advanced.resource_index.side[side].substitute[rtype].box)
			}
		}

		// create score manager
		window.kbmodules.futsal_advanced.resource_index.score_manager = new window.kbmodules.futsal_advanced.ScoreManager()
	}
	...
	// --------------------------
	{
		...
		$('#load_existing_club_dropdown label').append(
			window.kbmodules.futsal_advanced.resource_index.club_selector_dropdown.dropdown_elem(
				function(evt){
					window.kbmodules.futsal_advanced.load_club_by_name(evt.target.value)
				}
			)
		);
		...
		$('home-club-selector').append(
			window.kbmodules.futsal_advanced.resource_index.club_selector_dropdown.dropdown_elem(
				function(evt){
					window.kbmodules.futsal_advanced.create_club_lineup('home', evt.target.value);

					// todo: is this the right place for this?
					// Trigger lineup save
					window.kbmodules.futsal_advanced.global_save({'lineup_lists': true})
				}
			)
		);
		...
		$('guest-club-selector').append(
			window.kbmodules.futsal_advanced.resource_index.club_selector_dropdown.dropdown_elem(
				function(evt){
					window.kbmodules.futsal_advanced.create_club_lineup('guest', evt.target.value);
		...
		$('#cg_player_picker').append(
			window.kbmodules.futsal_advanced.resource_index.card_player_filter.box
		)
	}
	...
	// --------------------------
	{
		...
		window.kbmodules.futsal_advanced.update_team_colors();
		window.kbmodules.futsal_advanced.resource_index.score_manager.resync_score_on_title()
		...
		for (const img of qselAll('#weather_ctrl_body [icon_id]')){
			img.src = str(Path(
				window.kbmodules.futsal_advanced.resources_location.value,
				'icons',
	...
	//
	{
		...
		window.kbmodules.futsal_advanced.stats_unit_pool = {};
		...
		for (const stat_info of stats){
			...
			const stat_class = 
				new window.kbmodules.futsal_advanced.StatUnit(
					window.kbmodules.futsal_advanced.titles.stats,
					stat_info[1],
					stat_info[2],
					stat_info[3],
			...
			window.kbmodules.futsal_advanced.stats_unit_pool[stat_name] = stat_class;
		}
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.load_lineup_lists();
		window.kbmodules.futsal_advanced.load_field_layouts();
		window.kbmodules.futsal_advanced.load_card_data();
		window.kbmodules.futsal_advanced.load_score_data();
	}
	...
	// --------------------------
	{
		...
		const timer_sys_switch = new ksys.switches.KBSwitch({
			...
			// 'callback': window.kbmodules.futsal_advanced.update_selected_timer_system(sw, selected_id, event),
			'callback': window.kbmodules.futsal_advanced.update_selected_timer_system,
		})

		window.kbmodules.futsal_advanced.resource_index.timer_sys_switch = timer_sys_switch;
		...
		window.kbmodules.futsal_advanced.at_at = {
			'service_ping': null,
		...
		window.kbmodules.futsal_advanced.timer_ctrl = window.kbmodules.futsal_advanced.timer_fset[tgt_timer_fset];

		if (tgt_timer_fset == 'at_at'){
			window.kbmodules.futsal_advanced.toggle_atat_status_indicator(true);
			window.kbmodules.futsal_advanced.restart_atat_service();
			window.kbmodules.futsal_advanced.restore_atat_line();
		}else{
			window.kbmodules.futsal_advanced.toggle_atat_btns(true);
			window.kbmodules.futsal_advanced.toggle_atat_status_indicator(false);
			...
			ksys.ticker.bundestag.attach({
				'clock_id': window.kbmodules.futsal_advanced.main_clock_id,
				'tick':     window.kbmodules.futsal_advanced.timer_fset.builtin.timer_callback,
				'end':      window.kbmodules.futsal_advanced.timer_fset.builtin.base_timer_end_callback,
			})

			// Extra time
			ksys.ticker.bundestag.attach({
				'clock_id': window.kbmodules.futsal_advanced.extra_clock_id,
				'tick':     window.kbmodules.futsal_advanced.timer_fset.builtin.extra_timer_callback,
				'end':      window.kbmodules.futsal_advanced.timer_fset.builtin.extra_time_end_callback,
			})

			window.kbmodules.futsal_advanced.timer_fset.builtin.timer_callback(
				await ksys.ticker.bundestag.read_clock('fball.main')
			)
			window.kbmodules.futsal_advanced.timer_fset.builtin.extra_timer_callback(
				await ksys.ticker.bundestag.read_clock('fball.extra')
			)
		}

		// window.kbmodules.futsal_advanced.update_selected_timer_system(tgt_timer_fset);
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.resource_index.penalty_manager.resync(
			ksys.db.module.read('penalty_data.kbsave', 'json')
		);

		window.kbmodules.futsal_advanced.resource_index.penalty_manager.resync_vmix();
	}
	...
	// --------------------------
	{
		window.kbmodules.futsal_advanced.sequencer = new ksys.sequencer.TitleSequencer({
		...
		window.kbmodules.futsal_advanced.sequencer.load();

		document.querySelector('#sequencer').append(window.kbmodules.futsal_advanced.sequencer.dom.root);
	}
	...
	// --------------------------
	{
		...
		window.kbmodules.futsal_advanced.misc_titles = Object.freeze({
			'commenter': new window.kbmodules.futsal_advanced.MiscTitle({
				'title_id': 'commenter',
				'dom_list': [
			...
			'tape': new window.kbmodules.futsal_advanced.MiscTitle({
				'title_id': 'tape',
				'dom_list': [
			...
			'info_1': new window.kbmodules.futsal_advanced.MiscTitle({
				'title_id': 'info_1',
				'dom_list': [
			...
			'info_2': new window.kbmodules.futsal_advanced.MiscTitle({
				'title_id': 'info_2',
				'dom_list': [
			...
			'water_time': new window.kbmodules.futsal_advanced.MiscTitle({
				'title_id': 'water_time',
				'dom_list': [
			...
			'referee': new window.kbmodules.futsal_advanced.MiscTitle({
				'title_id': 'referee',
				'dom_list': [
			...
			'weather': new window.kbmodules.futsal_advanced.MiscTitle({
				'title_id': 'weather',
				'dom_list': [
	...
	// --------------------------
	{
		...
		window.kbmodules.futsal_advanced.resource_index.home_lineup.subs.add_pair([
			window.kbmodules.futsal_advanced.resource_index.home_lineup.main_players.at(0),
			window.kbmodules.futsal_advanced.resource_index.home_lineup.main_players.at(1),
		])
		window.kbmodules.futsal_advanced.resource_index.home_lineup.subs.add_pair([
			window.kbmodules.futsal_advanced.resource_index.home_lineup.main_players.at(2),
			window.kbmodules.futsal_advanced.resource_index.home_lineup.main_players.at(3),
		])
		window.kbmodules.futsal_advanced.resource_index.home_lineup.subs.add_pair([
			window.kbmodules.futsal_advanced.resource_index.home_lineup.main_players.at(4),
			window.kbmodules.futsal_advanced.resource_index.home_lineup.main_players.at(5),
		])

		window.kbmodules.futsal_advanced.resource_index.guest_lineup.subs.add_pair([
			window.kbmodules.futsal_advanced.resource_index.guest_lineup.main_players.at(0),
			window.kbmodules.futsal_advanced.resource_index.guest_lineup.main_players.at(1),
		])
		window.kbmodules.futsal_advanced.resource_index.guest_lineup.subs.add_pair([
			window.kbmodules.futsal_advanced.resource_index.guest_lineup.main_players.at(2),
			window.kbmodules.futsal_advanced.resource_index.guest_lineup.main_players.at(3),
		])
		// window.kbmodules.futsal_advanced.resource_index.guest_lineup.subs.add_pair([
		// 	window.kbmodules.futsal_advanced.resource_index.guest_lineup.main_players.at(4),
		// 	window.kbmodules.futsal_advanced.resource_index.guest_lineup.main_players.at(5),
		// ])
...
window.kbmodules.futsal_advanced.FootballClub = class{
	constructor(input_club_struct=null, is_enemy=false){
		...
		for (const player_info of (club_struct.playerbase || [])){
			self.playerbase.add(new window.kbmodules.futsal_advanced.ClubPlayer(this, player_info))
		}
	...
	erase(self){
		...
		if (window.kbmodules.futsal_advanced.resource_index.home_lineup?.club?.club_name?.lower?.() == self.club_name.lower()){
			$('#team_lineup home-club-lineup').empty()
		}
		if (window.kbmodules.futsal_advanced.resource_index.guest_lineup?.club?.club_name?.lower?.() == self.club_name.lower()){
	...
	register_player(self, input_player_info=null){
		// create player class
		const player = new window.kbmodules.futsal_advanced.ClubPlayer(self, input_player_info);
		...
		tplate.index.init_from_url_btn.onclick = async function(evt){
			...
			html_d.querySelectorAll('#ex1-tabs-1 .my-1').forEach(function(el) {
				...
				const pdata = self.register_player({
					...
					'role': window.kbmodules.futsal_advanced.role_editor.create({
	...
	open_panel(self){
		...
		window.kbmodules.futsal_advanced.resource_index.club_ctrl = self;
	}
...
window.kbmodules.futsal_advanced.ClubPlayer = class{
	...
	$surname(self){
		...
		if (window.kbmodules.futsal_advanced.pname_schema_switch.selected == 'prefixed'){
	...
	forward_update(self){
		...
		/*
		window.kbmodules.futsal_advanced.global_save({
			'lineup_lists': true,
			'field_layout': true,
		})
		*/
	...
	player_params_elem(self){
		...
		const role_selector_dom = window.kbmodules.futsal_advanced.role_editor.selector_dom(self);
		...
		tplate.elem.oncontextmenu = function(evt){
			const cant_del = [
				window.kbmodules.futsal_advanced.resource_index.side.home?.club?.club_name?.lower?.(),
				window.kbmodules.futsal_advanced.resource_index.side.guest?.club?.club_name?.lower?.(),
			]
	...
	$$role(self, tgt_role=null){
		if ((typeof tgt_role) == 'string'){
			self._role = window.kbmodules.futsal_advanced.role_editor.lookup(tgt_role);
			self.forward_update();
			return
		}
		self._role = tgt_role;
...
*/
window.kbmodules.futsal_advanced.TeamLineup = class{
	constructor(club, colors=null, input_lineup_info=null){
		...
		self.fouls = new window.kbmodules.futsal_advanced.TeamFouls(self);
		...
		// Substitute stack
		self.subs = new window.kbmodules.futsal_advanced.PlayerSubstitutes(self);
	}
	...
	control_panel_elem(self){
		...
		// 
		const player_picker = new window.kbmodules.futsal_advanced.PlayerPicker(
		...
		// 
		const resources = Path(window.kbmodules.futsal_advanced.resources_location.value || '/');


		// todo: shorts are now used for the timer title colours
		self.shorts_colpick = new window.kbmodules.futsal_advanced.TeamLineupColorPicker({
			'baking': {
		...
		self.tshirt_colpick = new window.kbmodules.futsal_advanced.TeamLineupColorPicker({
			'baking': {
		...
		self.gk_colpick = new window.kbmodules.futsal_advanced.TeamLineupColorPicker({
			'baking': {
				'mask': resources.join('tshirt', 'mask.png'),
				'bg': resources.join('tshirt', 'outline.png'),
...
*/
window.kbmodules.futsal_advanced._TeamLineupColorPicker = class{
...
window.kbmodules.futsal_advanced.TeamLineupColorPicker = class{
	...
	$baked_WORKS(self){
		return new Promise(async function(resolve, reject){
			...
			const result_fpath = Path(window.kbmodules.futsal_advanced.resources_location.value).join('baked', str(self.bake_params.result_name));

			// 1 - generate color fill
			self.nprint('Generating fill');
			const fill_fpath = Path(window.kbmodules.futsal_advanced.resources_location.value, 'baked', `${self.color.replaceAll('#', '')}.png`);
			const fill_params = [
				'-y',
				'-f', 'lavfi',
			...
			const mask_proc = child_proc(window.kbmodules.futsal_advanced.img_magick.value, [
			...
			if (self.bake_params.bg){
				...
				const overlay_proc = child_proc(window.kbmodules.futsal_advanced.img_magick.value, [
					// bottom gear
	...
	$baked_shorter(self){
		return new Promise(async function(resolve, reject){
			...
			const fill_proc_result = await self.run_proc(
				window.kbmodules.futsal_advanced.ffmpeg.value,
				fill_params
			)
			...
			const mask_proc_result = await self.run_proc(
				window.kbmodules.futsal_advanced.img_magick.value,
				mask_params
			)
...
*/
window.kbmodules.futsal_advanced.PlayerPicker = class{
	...
	match_query(self, query){
		...
		for (const dsource of self.data_source){
			for (const player of dsource){
				...
				window.kbmodules.futsal_advanced.role_editor.litter();
			}
		}
...
window.kbmodules.futsal_advanced.ClubSelectorDropdown = class{
	constructor(){
		const self = this;
		ksys.util.cls_pwnage.remap(self);
...
window.kbmodules.futsal_advanced.FieldLayout = class{
	constructor(lineup){
		const self = this;
		ksys.util.cls_pwnage.remap(self);
				...
				cell.dom.onmouseup = function(){
					...
					if (cell.player){
						...
						if (new_cell && old_cell){
							...
							// todo: is this the right place for this?
							// Save field layout
							window.kbmodules.futsal_advanced.global_save({'field_layout': true})
						}
		...
		// create player picker
		self.picker = new window.kbmodules.futsal_advanced.PlayerPicker(
			// data source
...
*/
window.kbmodules.futsal_advanced._FMStatUnit = class{
...
window.kbmodules.futsal_advanced._FMStats = class{
...
window.kbmodules.futsal_advanced.FMStatsTable = class{
...
// stat_map is a dict of column_id:value
window.kbmodules.futsal_advanced.FMStatInstance = class{
...
window.kbmodules.futsal_advanced.CardManager = class {
	constructor(init_data=null){
		...
		self.player_picker = window.kbmodules.futsal_advanced.resource_index.card_player_filter;
	...
	resync(self){
		for (const side of ['home', 'guest']){
			...
			const active_club = window.kbmodules.futsal_advanced.resource_index.side[side].club;
	...
	async show_card_title(self, title, player){
		if (window.kbmodules.futsal_advanced.title_schema_switch.selected == 'paged'){
			...
			if (title.title_name == 'yellow_card.gtzip'){
				...
				icon = Path(window.kbmodules.futsal_advanced.resources_location.value, 'icons/yellow_card.png')
			}

			if (title.title_name == 'ycbr.gtzip'){
				header_text = 'ДРУГА ЖОВТА КАРТКА';
				icon = Path(window.kbmodules.futsal_advanced.resources_location.value, 'icons/ycbr.png')
			}

			if (title.title_name == 'red_card.gtzip'){
				header_text = 'ЧЕРВОНА КАРТКА';
				icon = Path(window.kbmodules.futsal_advanced.resources_location.value, 'icons/red_card.png')
			}

			// window.kbmodules.futsal_advanced.timeout_persistent_header_btns(7000 + 2500 + 50);
			window.kbmodules.futsal_advanced.timeout_persistent_header_btns(
				window.kbmodules.futsal_advanced.sequencer.sequences.cards.sum
			);

			const time = window.kbmodules.futsal_advanced.get_current_time(true);
			const time_suffix = time.extra ? `+${time.extra}` : '';

			await window.kbmodules.futsal_advanced.persistent_header.event({
				'header': header_text,
			...
			await ksys.util.sleep(
				window.kbmodules.futsal_advanced.sequencer.sequences.cards.items.hold.dur
			);
			// Switch back to clock
			await window.kbmodules.futsal_advanced.persistent_header.clock();
			...
			ksys.btns.adv_timeout({
				'yellow_card': window.kbmodules.futsal_advanced.sequencer.sequences.cards.sum,
				'red_card':    window.kbmodules.futsal_advanced.sequencer.sequences.cards.sum,
			})

			// Hide red cards
			await window.kbmodules.futsal_advanced.hide_red_cards();
			...
			// await ksys.util.sleep(window.kbmodules.futsal_advanced.sequencer.sequences.cards.items.hold.dur);
			// hide the title
	...
	async hand_yellow(self, player){
		...
		if (pcard_info.warned){
			pcard_info.red = true;
			title = window.kbmodules.futsal_advanced.titles.ycbr_card;
			...
			title = window.kbmodules.futsal_advanced.titles.yellow_card;

			// todo: is this the right place?
			// save card data
			window.kbmodules.futsal_advanced.global_save({'card_data': true})
		}
	...
	async hand_red(self, player){
		...
		const title = window.kbmodules.futsal_advanced.titles.red_card;
	...
	async resync_vmix(self){
		...
		for (const _side_idx in retarded){
			...
			for (const card_idx of range(3)){
				if (window.kbmodules.futsal_advanced.red_card_schema_switch.selected == 'images'){
					await window.kbmodules.futsal_advanced.titles.timer.toggle_img(
						`rcard_${side_idx+1}_${card_idx+1}`,
						!!side.red_stack.at(card_idx)
					)
				}else{
					await window.kbmodules.futsal_advanced.titles.timer.set_shape_color(
						`rcard_${side_idx+1}_${card_idx+1}`,
						window.kbmodules.futsal_advanced.red_card_schema_color.value + (!!side.red_stack.at(card_idx) ? '' : '00')
					)
				}
...
window.kbmodules.futsal_advanced.ClubGoals = class {
	constructor(parent_club, init_data=null){
		...
		*/
		...
		self.tplate.index.add_score_named_btn.onclick = function(evt){
			const selected_player = window.kbmodules.futsal_advanced.resource_index.score_manager.player_picker?.selected_entry?.player;
			if (!selected_player){
				ksys.info_msg.send_msg(
					`No player selected !`,
					'warn',
		...
		locked=false)
	{
		...
		// sanity check
		// if (!window.kbmodules.futsal_advanced?.base_counter?.tick && !ignore_timer){
		if (false){
			ksys.info_msg.send_msg(
		...
		const calculated_timestamp = timestamp_override || window.kbmodules.futsal_advanced.get_current_time(true);
		...
		tplate.index.timestamp.onchange = function(evt){
			...
			// sanity check
			const dur = window.kbmodules.futsal_advanced.round_duration;
			...
			window.kbmodules.futsal_advanced.global_save({'scores': true});
		}
		...
		tplate.elem.onclick = function(evt){
			...
			window.kbmodules.futsal_advanced.resource_index.score_manager.reset_player_selection();
	...
	mod_author(self){
		...
		const new_player = window.kbmodules.futsal_advanced.resource_index.score_manager.player_picker.selected_entry.player;
...
window.kbmodules.futsal_advanced.ScoreManager = class{
	constructor(init_data=null){
		const self = this;
		ksys.util.cls_pwnage.remap(self);
		...
		self.player_picker = new window.kbmodules.futsal_advanced.PlayerPicker(
			[[]],
			function(){return true}
		)
	...
	resync_picker_sources(self){
		...
		for (const side of ['home', 'guest']){
			...
			new_source.push(window.kbmodules.futsal_advanced.resource_index.side[side]?.lineup?.main_players || []);
			new_source.push(window.kbmodules.futsal_advanced.resource_index.side[side]?.lineup?.reserve_players || []);
		}
	...
	resync_lineups(self){
		...
		for (const side of ['home', 'guest']){
			const club_data = window.kbmodules.futsal_advanced.resource_index.side[side]
			if (club_data.lineup){
				const club_goals = new window.kbmodules.futsal_advanced.ClubGoals(club_data.club);
	...
	reset_player_selection(self){
		...
		for (const side of ['home', 'guest']){
			const score_list = window.kbmodules.futsal_advanced.resource_index.score_manager.sides[side].score_list;
	...
	async resync_score_on_title(self){
		const title = window.kbmodules.futsal_advanced.titles.timer;

		// important todo: keep doing these stupid checks or simply lock all tabs till both lineups are present ?
		const home_score_list = window.kbmodules.futsal_advanced.resource_index.score_manager.sides?.home?.score_list
		if (home_score_list){
			await title.set_text('score_l', home_score_list.score_stack.size)
		}

		const guest_score_list = window.kbmodules.futsal_advanced.resource_index.score_manager.sides?.guest?.score_list
		if (guest_score_list){
...
window.kbmodules.futsal_advanced.PenaltyManager = class{
	...
	resync(self, input_data=null){
		...
		self.sides = {
			'home': new window.kbmodules.futsal_advanced.TeamPenaltyPool(
				self,
				window.kbmodules.futsal_advanced.resource_index.score_manager.sides['home'],
				window.kbmodules.futsal_advanced.resource_index.side.home.club,
				input_data?.['home']
			),
			'guest': new window.kbmodules.futsal_advanced.TeamPenaltyPool(
				self,
				window.kbmodules.futsal_advanced.resource_index.score_manager.sides['guest'],
				window.kbmodules.futsal_advanced.resource_index.side.guest.club,
				input_data?.['guest']
			),
		}
	...
	async resync_vmix(self){
		...
		await window.kbmodules.futsal_advanced.titles.penalties.set_img_src(
			'club_logo_l',
			window.kbmodules.futsal_advanced.resource_index.side.home.club.logo_path
		)
		await window.kbmodules.futsal_advanced.titles.penalties.set_img_src(
			'club_logo_r',
			window.kbmodules.futsal_advanced.resource_index.side.guest.club.logo_path
		)

		await window.kbmodules.futsal_advanced.titles.penalties.set_shape_color(
			'color_l',
			window.kbmodules.futsal_advanced.resource_index.side.home.lineup.tshirt_colpick.color
		)
		await window.kbmodules.futsal_advanced.titles.penalties.set_shape_color(
			'color_r',
			window.kbmodules.futsal_advanced.resource_index.side.guest.lineup.tshirt_colpick.color
		)

		await window.kbmodules.futsal_advanced.titles.penalties.set_text(
			'team_name_r',
			window.kbmodules.futsal_advanced.resource_index.side.guest.club.club_name_shorthand.upper()
		)
		await window.kbmodules.futsal_advanced.titles.penalties.set_text(
			'team_name_l',
			window.kbmodules.futsal_advanced.resource_index.side.home.club.club_name_shorthand.upper()
		)

		await window.kbmodules.futsal_advanced.titles.penalties.set_text(
			`score_l`,
			// window.kbmodules.futsal_advanced.resource_index.score_manager.sides['home'].score_list.score_stack.size
			self.sides.home.sum
		)
		await window.kbmodules.futsal_advanced.titles.penalties.set_text(
			`score_r`,
			// window.kbmodules.futsal_advanced.resource_index.score_manager.sides['guest'].score_list.score_stack.size
...
window.kbmodules.futsal_advanced.TeamPenaltyPool = class{
	constructor(parent_manager, score_manager, tgt_team, input_data=null){
		...
		for (const idx of range(self.parent_manager.pcount)){
			const ctrl_elem = new window.kbmodules.futsal_advanced.PenaltyTableEntry(
				self,
				idx,
				input_data?.[str(idx)]
			);
...
window.kbmodules.futsal_advanced.PenaltyTableEntry = class{
	constructor(parent_pool, penalty_idx, input_data=null){
		...
		self.dom_data.index.cbox_yes.onchange = function(){
			...
			window.kbmodules.futsal_advanced.save_penalties();
		}
	...
	push_to_vmix(self, dosave=false){
		const tgt_title = window.kbmodules.futsal_advanced.titles.penalties;
		...
		// penalties
		// let tgt_icon = Path(window.kbmodules.futsal_advanced.resources_location.value, 'icons/yellow_card.png');
		let tgt_icon = Path(window.kbmodules.futsal_advanced.resources_location.value, 'icons/p_neutral.png');

		if (self.dom_data.index.cbox_no.checked){
			tgt_icon = Path(window.kbmodules.futsal_advanced.resources_location.value, 'icons/p_no.png');
		}

		if (self.dom_data.index.cbox_yes.checked){
			tgt_icon = Path(window.kbmodules.futsal_advanced.resources_location.value, 'icons/p_yes.png');
		}
...
window.kbmodules.futsal_advanced.PersistentHeaderPageManager = class{
	...
	async pull_page_from_vmix(self, apply=true){
		const title_data = await window.kbmodules.futsal_advanced.titles.timer.pull_xml();
		if (!title_data){
			return null
		}
	...
	seek(self, target){
		...
		// 		if (allowed == tgt){
		// 			self.current = tgt;
		// 			await window.kbmodules.futsal_advanced.titles.timer.page(tgt, self.TR_DUR[tgt]);
		// 			found = true;
		// 			break
		// 		}
		// 	}
	...
	async transition(self, target=null){
		for (const attempt of range(3)){
			if (attempt == 0){
				...
				await window.kbmodules.futsal_advanced.titles.timer.page(
					direct_switch,
...
window.kbmodules.futsal_advanced.PersistentHeader = class{
	...
	constructor(){
		...
		self.pages = new window.kbmodules.futsal_advanced.PersistentHeaderPageManager();
		self.title = window.kbmodules.futsal_advanced.titles.timer;
	}
...
window.kbmodules.futsal_advanced.PlayerRoleInstance = class{
	...
	remove(self){
		const home_playerbase = window.kbmodules.futsal_advanced.resource_index?.home_club?.playerbase || new Set();
		const guest_playerbase = window.kbmodules.futsal_advanced.resource_index?.guest_club?.playerbase || new Set();
...
window.kbmodules.futsal_advanced.PlayerRolesEditor = class{
	...
	create(self, data=null, check_duplicates=true){
		...
		const role = new window.kbmodules.futsal_advanced.PlayerRoleInstance(self, data);
		self.roles.add(role);
...
window.kbmodules.futsal_advanced.PlayerSubstitutes = class{
	constructor(lineup){
		const self = ksys.util.nprint(
			ksys.util.cls_pwnage.remap(this),
			'#FFF5A1',
	...
	async run_title_single(self, players, subs_type='break'){
		...
		if (subs_type == 'match'){
			if (window.kbmodules.futsal_advanced.title_schema_switch.selected == 'paged'){
				...
				window.kbmodules.futsal_advanced.timeout_persistent_header_btns(
					window.kbmodules.futsal_advanced.sequencer.sequences.subs_single_match.sum
				);
				// Persistent header class applies data and switches to the corrent page on its own,
				// BUT it does NOT switch back to clock
				await window.kbmodules.futsal_advanced.persistent_header.subs({
				...
				await ksys.util.sleep(window.kbmodules.futsal_advanced.sequencer.sequences.subs_single_match.items.hold.dur);
				// Switch back to clock, because this doesn't happen automatically for good reasons
				await window.kbmodules.futsal_advanced.persistent_header.clock();
			}else{
				// Set Logo
				await window.kbmodules.futsal_advanced.titles.m_subs1.set_img_src(
					'club_logo',
					self.lineup.club.logo_path
				);

				// NAMES - LEAVING
				await window.kbmodules.futsal_advanced.titles.m_subs1.set_text(
					`pnum_leaving0`,
					leaving.player_num
				);
				await window.kbmodules.futsal_advanced.titles.m_subs1.set_text(
				...
				// Show the title
				await window.kbmodules.futsal_advanced.titles.m_subs1.overlay_in();

				// Hold the title for n seconds
				await ksys.util.sleep(window.kbmodules.futsal_advanced.sequencer.sequences.subs_single_match.items.hold.dur);

				// Hide the title
				await window.kbmodules.futsal_advanced.titles.m_subs1.overlay_out();
		...
		if (subs_type == 'break'){
			ksys.btns.adv_timeout({
				'exec_replacement_sequence_stack_break_home': window.kbmodules.futsal_advanced.sequencer.sequences.subs_stack_break.sum,
				'exec_replacement_sequence_stack_break_guest': window.kbmodules.futsal_advanced.sequencer.sequences.subs_stack_break.sum,
				'exec_replacement_sequence_break': window.kbmodules.futsal_advanced.sequencer.sequences.subs_single_break.sum,
			})

			// Set Logo
			await window.kbmodules.futsal_advanced.titles.subs1.set_img_src(
				'club_logo',
				leaving.club.logo_path
			);

			// NAMES - LEAVING
			await window.kbmodules.futsal_advanced.titles.subs1.set_text(
				'pnum_leaving0',
				leaving.player_num
			);
			await window.kbmodules.futsal_advanced.titles.subs1.set_text(
			...
			// Show the title
			await window.kbmodules.futsal_advanced.titles.subs1.overlay_in();


			// Hold the title for n seconds
			await ksys.util.sleep(window.kbmodules.futsal_advanced.sequencer.sequences.subs_single_break.items.hold.dur);
			// Hide the title
			await window.kbmodules.futsal_advanced.titles.subs1.overlay_out();
		}
	}
	...
	async run_title_stack(self, subs_type='break'){
		...
		if (subs_type == 'match'){
			if (window.kbmodules.futsal_advanced.title_schema_switch.selected == 'paged'){
				// Persistent Header can only do one thing at a time
				window.kbmodules.futsal_advanced.timeout_persistent_header_btns(
					window.kbmodules.futsal_advanced.sequencer.sequences.subs_stack_match.sum
				);
				...
				// Hold the title for n seconds
				await ksys.util.sleep(window.kbmodules.futsal_advanced.sequencer.sequences.subs_stack_match.items.hold.dur);
				// Switch back to clock, because this doesn't happen automatically for good reasons
				await window.kbmodules.futsal_advanced.persistent_header.clock();
			}else{
				// important todo: this is fucking retarded
				for (const tile_id of ['m_subs1', 'm_subs2', 'm_subs3']){
					const tgt_title = window.kbmodules.futsal_advanced.titles[tile_id];
				...
				const tgt_title = window.kbmodules.futsal_advanced.titles[`m_subs${self.stack.size}`];
		...
		if (subs_type == 'break'){
			...
			// Select the appropriate title
			const tgt_title = window.kbmodules.futsal_advanced.titles[`subs${self.stack.size}`];

			// Show the appropriate title
			await tgt_title.overlay_in();

			// Hold the title for n seconds
			await ksys.util.sleep(window.kbmodules.futsal_advanced.sequencer.sequences.subs_stack_break.items.hold.dur);
...
window.kbmodules.futsal_advanced.MiscTitle = class{
	...
	bind_field(self, tgt_field, set_data=null){
		...
		tgt_field.onchange = function(){
			window.kbmodules.futsal_advanced.global_save({'misc_titles': true});
		}
	}
...
window.kbmodules.futsal_advanced.TeamFouls = class{
	MAX_FOULS = 5;
	...
	async redrawVMIX(self){
		await window.kbmodules.futsal_advanced.titles.timer.set_img_src(
			'foul_' + (self.lineup.club.is_enemy ? 'r' : 'l'),
			Path(window.kbmodules.futsal_advanced.resources_location.value, 'fouls', `${self.foulsAmount}.png`)
		)
	}
}
...
window.kbmodules.futsal_advanced.create_new_club = function(club_resources=null, open_panel=true){
	window.kbmodules.futsal_advanced.save_club_to_local_db()

	// create new club class
	const new_club = new window.kbmodules.futsal_advanced.FootballClub(club_resources);
	// write club reference to the registry
	window.kbmodules.futsal_advanced.resource_index.club_ctrl = new_club;
	new_club.open_panel()
}

// Save file to an abstract location on disk
window.kbmodules.futsal_advanced.save_club_to_file = function(){
	...
	if (!tgt_dir || !window.kbmodules.futsal_advanced.resource_index.club_ctrl){
	...
	tgt_file.writeFileSync(
		JSON.stringify(window.kbmodules.futsal_advanced.resource_index.club_ctrl.to_json(), null, '\t')
	)

	// todo: is this the right place for it?
	// see explanation in save_club_to_local_db about why is it here
	window.kbmodules.futsal_advanced.global_save(null)
}


// Delete currently edited club from disk
// and wipe anything referencing that club from the interface
window.kbmodules.futsal_advanced.delete_current_club = function(evt){
	if (!evt.ctrlKey){return};

	const club = window.kbmodules.futsal_advanced.resource_index.club_ctrl;
	if (!club){return};
	...
	// 3 - resync dropdowns
	window.kbmodules.futsal_advanced.resource_index.club_selector_dropdown.resync()
}


window.kbmodules.futsal_advanced.save_club_to_local_db = function(mute=true){
	// Make sure there's a club to save
	if (!window.kbmodules.futsal_advanced.resource_index?.club_ctrl?.club_name){
		if (!mute){
	...
	const club_info = window.kbmodules.futsal_advanced.resource_index.club_ctrl.to_json();
...
window.kbmodules.futsal_advanced.get_club_info_by_name = function(clubname=null){
	if (!clubname){return};
...
window.kbmodules.futsal_advanced.load_club_by_name = function(clubname=null){
	if (!clubname){return};

	const existing_home = window.kbmodules.futsal_advanced.resource_index.home_club;
	const existing_guest = window.kbmodules.futsal_advanced.resource_index.guest_club;
	...
	window.kbmodules.futsal_advanced.create_new_club(window.kbmodules.futsal_advanced.get_club_info_by_name(clubname))
}
...
window.kbmodules.futsal_advanced.create_club_lineup = function(side, clubname, input_lineup_info=null){
	if (!clubname){
	...
	if (window.kbmodules.futsal_advanced.resource_index.club_ctrl?.club_name?.lower?.() == clubname.lower()){
		// if so - get the Club class reference from the resource index
		club = window.kbmodules.futsal_advanced.resource_index.club_ctrl;
		// and update the is_enemy flag
		window.kbmodules.futsal_advanced.resource_index.club_ctrl.is_enemy = (side == 'guest');

	}else{
		// if not - create new Club class,
		// because a lineup cannot exist without a club
		// Lineup class is always a parent of a Club class
		club = new window.kbmodules.futsal_advanced.FootballClub(
			window.kbmodules.futsal_advanced.get_club_info_by_name(clubname),
			side == 'guest'
		);
	}

	// create player lineup for the club
	const lineup = new window.kbmodules.futsal_advanced.TeamLineup(
		// parent club
		club,
		// available uniform colours
		window.kbmodules.futsal_advanced.resource_index.available_colors,
		// input info (player lists, field layout, etc ...)
		input_lineup_info
	)

	// create field layout
	const field_layout = new window.kbmodules.futsal_advanced.FieldLayout(lineup)

	if (side == 'home'){
		// write down the club to the resource index
		window.kbmodules.futsal_advanced.resource_index.home_club = club;
		// write down lineup to the resource index
		window.kbmodules.futsal_advanced.resource_index.home_lineup = lineup;
		// write field reference to the resource index
		window.kbmodules.futsal_advanced.resource_index.home_field = field_layout;
	}

	if (side == 'guest'){
		// write down the club to the resource index
		window.kbmodules.futsal_advanced.resource_index.guest_club = club;
		// write down lineup to the resource index
		window.kbmodules.futsal_advanced.resource_index.guest_lineup = lineup;
		// write field reference to the resource index
		window.kbmodules.futsal_advanced.resource_index.guest_field = field_layout;
	}

	// New fancy system
	// (should've been like that from the very beginning)
	if (['home', 'guest'].includes(side)){
		window.kbmodules.futsal_advanced.resource_index.side[side].club = club;
		window.kbmodules.futsal_advanced.resource_index.side[side].lineup = lineup;
		window.kbmodules.futsal_advanced.resource_index.side[side].field = field_layout;
	}
	...
	window.kbmodules.futsal_advanced.resource_index.card_player_filter.data_source = [
		// (window.kbmodules.futsal_advanced.resource_index.home_lineup ? window.kbmodules.futsal_advanced.resource_index.home_lineup.main_players : []),
		// (window.kbmodules.futsal_advanced.resource_index.home_lineup ? window.kbmodules.futsal_advanced.resource_index.home_lineup.reserve_players : []),

		// (window.kbmodules.futsal_advanced.resource_index.guest_lineup ? window.kbmodules.futsal_advanced.resource_index.guest_lineup.main_players : []),
		// (window.kbmodules.futsal_advanced.resource_index.guest_lineup ? window.kbmodules.futsal_advanced.resource_index.guest_lineup.reserve_players : []),

		(window.kbmodules.futsal_advanced.resource_index?.home_lineup?.main_players || []),
		(window.kbmodules.futsal_advanced.resource_index?.home_lineup?.reserve_players || []),

		(window.kbmodules.futsal_advanced.resource_index?.guest_lineup?.main_players || []),
		(window.kbmodules.futsal_advanced.resource_index?.guest_lineup?.reserve_players || []),
	]

	// update sources for substitutes
	for (const side of ['home', 'guest']){
		for (const rtype of ['leaving', 'inbound']){
			window.kbmodules.futsal_advanced.resource_index.side[side].substitute[rtype].data_source = [
				(window.kbmodules.futsal_advanced.resource_index.side[side]?.lineup?.main_players || []),
				(window.kbmodules.futsal_advanced.resource_index.side[side]?.lineup?.reserve_players || []),
			]
		}
	}

	// update sources for score picker
	window.kbmodules.futsal_advanced.resource_index.score_manager.resync_picker_sources()

	// resync score manager
	window.kbmodules.futsal_advanced.resource_index.score_manager.resync_lineups()

	// resync card manager
	window.kbmodules.futsal_advanced.resource_index.card_manager.resync()
	...
	{
		if (window.kbmodules.futsal_advanced.resource_index.side.home.club){
			$('#team_stats_theader_home').html(window.kbmodules.futsal_advanced.resource_index.side.home.club.vis_header_elem());
			$('#team_stats_btn_ctrl_home').html(window.kbmodules.futsal_advanced.resource_index.side.home.club.vis_header_elem());
			$('#replacement_team1 .replacement_team_head').html(window.kbmodules.futsal_advanced.resource_index.side.home.club.vis_header_elem());
		}
		if (window.kbmodules.futsal_advanced.resource_index.side.guest.club){
			$('#team_stats_theader_guest').html(window.kbmodules.futsal_advanced.resource_index.side.guest.club.vis_header_elem());
			$('#team_stats_btn_ctrl_guest').html(window.kbmodules.futsal_advanced.resource_index.side.guest.club.vis_header_elem());
			$('#replacement_team2 .replacement_team_head').html(window.kbmodules.futsal_advanced.resource_index.side.guest.club.vis_header_elem());
		}		
	}
...
window.kbmodules.futsal_advanced.update_team_colors = async function(){
	const home = window.kbmodules.futsal_advanced.resource_index.home_lineup;
	const guest = window.kbmodules.futsal_advanced.resource_index.guest_lineup;

	if (home){
		if (window.kbmodules.futsal_advanced.timer_team_color_schema_switch.selected == 'bitmap'){
			...
			await window.kbmodules.futsal_advanced.titles.final_scores.set_shape_color(
				'tshirt_color_l',
...
window.kbmodules.futsal_advanced.wipe_player_list_from_title = async function(){
	for (const idx of range(1, 12)){
		// player number
		await window.kbmodules.futsal_advanced.titles.team_layout.set_text(`plist_num_${idx}`, '')
		// player surname
		await window.kbmodules.futsal_advanced.titles.team_layout.set_text(`plist_pname_${idx}`, '');
	}
	await ksys.util.sleep(500)
}

window.kbmodules.futsal_advanced._forward_field_layout_to_vmix = async function(team){
	const tgt_side = str(team).lower();
	const tgt_field = window.kbmodules.futsal_advanced.resource_index.side?.[tgt_side]?.field;
	const suffix = window.kbmodules.futsal_advanced.resource_index.side?.[tgt_side]?.club?.is_enemy ? 'guest' : 'home';
	...
	const title = window.kbmodules.futsal_advanced.titles[`team_layout_${suffix}`];
	...
	const template_gtz = Path(window.kbmodules.futsal_advanced.field_layout_template_fpath.value);
	...
	await title.set_text(
		'roster',
		window.kbmodules.futsal_advanced.resource_index.side?.[tgt_side]?.lineup?.roster || ''
	)
	...
	// -------------------
	if (window.kbmodules.futsal_advanced.cmd_layout_mains_schema_switch.selected == 'dedicated'){
		const data_mapping = [
			{
	...
	if (window.kbmodules.futsal_advanced.cmd_layout_mains_schema_switch.selected == 'shared'){
	...
	if (window.kbmodules.futsal_advanced.cmd_layout_reserves_schema_switch.selected == 'shared'){
		// important todo: this is shit
		const list_nums = [];
		const list_names = [];
		for (const player_idx of range(11)){
	...
	if (window.kbmodules.futsal_advanced.cmd_layout_reserves_schema_switch.selected == 'dedicated'){
		const data_mapping = [
			// {
			// 	'plist': tgt_field.lineup.main_players,
			// 	'slot_type': 'm',
...
window.kbmodules.futsal_advanced.show_field_layout = async function(team){
	const tgt_side = str(team).lower();
	const tgt_field = window.kbmodules.futsal_advanced.resource_index.side?.[tgt_side]?.field;
	const suffix = window.kbmodules.futsal_advanced.resource_index.side?.[tgt_side]?.club?.is_enemy ? 'guest' : 'home';
	const kbnc = ksys.kbnc.KBNC.sysData().currentClient;
	...
	// get target vmix title
	const title = window.kbmodules.futsal_advanced.titles.team_layout;
...
window.kbmodules.futsal_advanced._show_field_layout = async function(team){
	ksys.btns.pool[`show_${team}_field_layout`].toggle(false)

	const tgt_title = window.kbmodules.futsal_advanced.titles[`team_layout_${team}`];

	await tgt_title.overlay_in()

	await ksys.util.sleep(
		window.kbmodules.futsal_advanced.sequencer.sequences.field_layout.items.hold_main.dur
	);
...
window.kbmodules.futsal_advanced.hide_field_layout = async function(){
	ksys.btns.toggle({
		'hide_home_field_layout':    false,
		'hide_guest_field_layout':   false,
	})
	await window.kbmodules.futsal_advanced.titles.team_layout.overlay_out();
	ksys.btns.toggle({
...
window.kbmodules.futsal_advanced._show_field_layout_condensed = async function(team){
	const tgt_side = str(team).lower();
	const tgt_lineup = window.kbmodules.futsal_advanced.resource_index.side?.[tgt_side]?.lineup;
	const role_gropus = {};
		...
		for (const x_idx of range(3)){
			for (const p_idx of range(4)){
				for (const nn of ['num', 'name']){
					await window.kbmodules.futsal_advanced.titles.lineup_condensed.toggle_text(
	...
	for (const grp_idx of range(grp_amount)){
		...
		// Apply group label
		await window.kbmodules.futsal_advanced.titles.lineup_condensed.set_text(
			`c${grp_idx}_label`,
			window.kbmodules.futsal_advanced.role_editor.lookup(role_id).role_name_full || ''
		)
	...
	// Goalkeeper
	const gk_data = window.kbmodules.futsal_advanced.resource_index.side?.[tgt_side]?.field?.goalkeeper;
	for (const nn of ['num', 'name']){
		// GK Num
		await window.kbmodules.futsal_advanced.titles.lineup_condensed.set_text(
			'gk_num',
	...
	// Show title
	await window.kbmodules.futsal_advanced.titles.lineup_condensed.overlay_in();
	...
	for (const grp_idx of range(grp_amount)){
		await ksys.util.sleep(window.kbmodules.futsal_advanced.sequencer.sequences.lineup_condensed.items.hold_page.dur);
		await window.kbmodules.futsal_advanced.titles.lineup_condensed.page(grp_idx + 1);
	}

	for (const grp_data of grp_array){
		const [role_id, plist] = grp_data;
		for (const player of plist){
			print(window.kbmodules.futsal_advanced.role_editor.lookup(role_id).role_name_full, ':', player.surname);
		}
	}

}

window.kbmodules.futsal_advanced.show_field_layout_condensed = async function(team){
	const max_per_page = int(window.kbmodules.futsal_advanced.lineup_condensed_max_players_per_page.selected);
	const tgt_side = str(team).lower();
	const tgt_lineup = window.kbmodules.futsal_advanced.resource_index.side?.[tgt_side]?.lineup;
	const role_gropus = {};
	...
	for (const grp_data of Object.entries(role_gropus)){
		...
		const role_data = window.kbmodules.futsal_advanced.role_editor.lookup(role_id);
	...
	await window.kbmodules.futsal_advanced.titles.lineup_condensed.resume_render();
	...
	for (const grp_data of role_groups_sorted){
		...
		while (plr_array.length){
			...
			if (title_shown){
				await window.kbmodules.futsal_advanced.titles.lineup_condensed.pause_render();
			}
			...
			const current_role_grp = (window.kbmodules.futsal_advanced.role_editor.lookup(role_id).role_name_alt || '').upper() + ':';
			...
			if (last_role_grp != current_role_grp){
				...
				await window.kbmodules.futsal_advanced.titles.lineup_condensed.set_text(
					...
					(window.kbmodules.futsal_advanced.role_editor.lookup(role_id).role_name_alt || '').upper() + ':'
				)
			}

			last_role_grp = current_role_grp;

			print('GRP LABEL:', (window.kbmodules.futsal_advanced.role_editor.lookup(role_id).role_name_alt || '') + ':')
			...
			if (!title_shown){
				...
				await window.kbmodules.futsal_advanced.titles.lineup_condensed.overlay_in();
				await ksys.util.sleep(
					window.kbmodules.futsal_advanced.sequencer.sequences.lineup_condensed.items.anim_in.dur
				);
			}
			...
			// Hold the page for n seconds
			await ksys.util.sleep(
				window.kbmodules.futsal_advanced.sequencer.sequences.lineup_condensed.items.hold_page.dur
			);
		}
	}
	...
	await window.kbmodules.futsal_advanced.titles.lineup_condensed.overlay_out();
	await ksys.util.sleep(
		window.kbmodules.futsal_advanced.sequencer.sequences.lineup_condensed.items.anim_out.dur
	);
...
window.kbmodules.futsal_advanced.hide_field_layout_condensed = async function(team){
	await window.kbmodules.futsal_advanced.titles.lineup_condensed.overlay_out();
}

window.kbmodules.futsal_advanced.update_title_schema = function(schema_id){
	ksys.context.module.prm('title_schema', schema_id);
}

window.kbmodules.futsal_advanced.timeout_persistent_header_btns = function(dur){
	ksys.btns.adv_timeout({
		'yellow_card':     dur,
		'red_card':        dur,
		'scored':          dur,
...
window.kbmodules.futsal_advanced.save_match_results = function(){
	const match_data = {
		'date': str(new Date),
		'home': {
			'club_name': window.kbmodules.futsal_advanced.resource_index.side.home.club.club_name,
			'score': window.kbmodules.futsal_advanced.resource_index.score_manager.sides.home.score_list.score_stack.size,
		},
		'guest': {
			'club_name': window.kbmodules.futsal_advanced.resource_index.side.guest.club.club_name,
			'score': window.kbmodules.futsal_advanced.resource_index.score_manager.sides.guest.score_list.score_stack.size,
		},
	}
...
//        Misc. Titles
// ================================
window.kbmodules.futsal_advanced.show_misc_title = async function(tgt_title){
	if (tgt_title == 'tape'){
		const tape_words = [];
		for (const word of window.kbmodules.futsal_advanced.misc_titles.tape.fields.tape.value.split('\n')){
			tape_words.push(word.trim());
		}
		await window.kbmodules.futsal_advanced.titles.tape.set_text(
			'text',
			tape_words.join(' ')
		)
		await window.kbmodules.futsal_advanced.titles.tape.overlay_in();
	}
	if (tgt_title == 'commenter'){
		const name_1 = window.kbmodules.futsal_advanced.misc_titles.commenter.fields.commenter_name_1.value.trim();
		const name_2 = window.kbmodules.futsal_advanced.misc_titles.commenter.fields.commenter_name_2.value.trim();

		// 2 girls 1 cup
		await window.kbmodules.futsal_advanced.titles.commenter.set_text(
			'name',
			name_1
		)
		await window.kbmodules.futsal_advanced.titles.commenter.set_text(
		...
		// 2 girls 2 cups
		await window.kbmodules.futsal_advanced.titles.commenter_x2.set_text(
			'name',
			name_1
		)
		await window.kbmodules.futsal_advanced.titles.commenter_x2.set_text(
		...
		if (window.kbmodules.futsal_advanced.commenter_schema_switch.selected == '2girls2cups'){
			if (name_1 && name_2){
				await window.kbmodules.futsal_advanced.titles.commenter_x2.overlay_in();
			}else{
				await window.kbmodules.futsal_advanced.titles.commenter.overlay_in();
			}
		}else{
			await window.kbmodules.futsal_advanced.titles.commenter.overlay_in();
		}
	...
	if (tgt_title == 'info_1'){
		await window.kbmodules.futsal_advanced.titles.info_1.set_text(
			'text',
			window.kbmodules.futsal_advanced.misc_titles.info_1.fields.info_1.value
		)
		await window.kbmodules.futsal_advanced.titles.info_1.overlay_in();
	}
	if (tgt_title == 'info_2'){
		await window.kbmodules.futsal_advanced.titles.info_2.set_text(
			'text_upper',
			window.kbmodules.futsal_advanced.misc_titles.info_2.fields.info_2_upper.value
		)
		await window.kbmodules.futsal_advanced.titles.info_2.set_text(
			'text_lower',
			window.kbmodules.futsal_advanced.misc_titles.info_2.fields.info_2_lower.value
		)
		await window.kbmodules.futsal_advanced.titles.info_2.overlay_in();
	}
	if (tgt_title == 'referee'){
		...
		for (const idx of range(1, 5)){
			const [name, title] = [
				window.kbmodules.futsal_advanced.misc_titles.referee.fields[`referee_${idx}_upper`].value.upper(),
				window.kbmodules.futsal_advanced.misc_titles.referee.fields[`referee_${idx}_lower`].value.upper(),
			];

			jobTitles.push(title);
			refereeNames.push(name);

			await window.kbmodules.futsal_advanced.titles.referee.set_text(
				`name_${idx}`,
				name
			)
			await window.kbmodules.futsal_advanced.titles.referee.set_text(
		...
		await window.kbmodules.futsal_advanced.titles.referee.overlay_in();
	}
	if (tgt_title == 'water_time'){
		await window.kbmodules.futsal_advanced.titles.water_time.set_text(
			'lower_text',
			window.kbmodules.futsal_advanced.misc_titles.water_time.fields.water_time_lower_text.value
		)
		await window.kbmodules.futsal_advanced.titles.water_time.overlay_in();
	}
	if (tgt_title == 'weather'){
		if (ksys.context.module.cache.weather_icon){
			await window.kbmodules.futsal_advanced.titles.weather.set_img_src(
				'clouds',
				str(Path(
					window.kbmodules.futsal_advanced.resources_location.value,
					'icons',
		...
		await window.kbmodules.futsal_advanced.titles.weather.set_text(
			'header_lower',
			(window.kbmodules.futsal_advanced.misc_titles.weather.fields.weather_header_lower.value || '')
		)
		await window.kbmodules.futsal_advanced.titles.weather.set_text(
			'temp',
			(window.kbmodules.futsal_advanced.misc_titles.weather.fields.weather_param_temperature.value || '') + '°'
		)
		await window.kbmodules.futsal_advanced.titles.weather.set_text(
			'humidity',
			(window.kbmodules.futsal_advanced.misc_titles.weather.fields.weather_param_humidity.value || '') + '%'
		)
		await window.kbmodules.futsal_advanced.titles.weather.set_text(
			'wind',
			(window.kbmodules.futsal_advanced.misc_titles.weather.fields.weather_param_wind_speed.value || '') + ' М/С'
		)



		await window.kbmodules.futsal_advanced.titles.weather.overlay_in();
	}
}

window.kbmodules.futsal_advanced.hide_misc_title = async function(tgt_title){
	for (const t_id of (Array.isArray(tgt_title) ? tgt_title : [tgt_title])){
		await window.kbmodules.futsal_advanced.titles[t_id].overlay_out();
	}
}

window.kbmodules.futsal_advanced.set_referee_to_default = function(){
	const titles = [
		'ГОЛОВНИЙ АРБІТР',
		'ПОМІЧНИК АРБІТРА',
		'ПОМІЧНИК АРБІТРА',
	...
	for (const idx of range(4)){
		window.kbmodules.futsal_advanced.misc_titles.referee.fields[`referee_${idx+1}_lower`].value = titles[idx];
	}
...
//        VS stuff
// ================================
window.kbmodules.futsal_advanced.save_vs_sublines = function(){
...
window.kbmodules.futsal_advanced.show_vs_title = async function(){

	if (!window.kbmodules.futsal_advanced.resource_index.side.home.club || !window.kbmodules.futsal_advanced.resource_index.side.guest.club){
		ksys.info_msg.send_msg(
	...
	await window.kbmodules.futsal_advanced.titles.splash.set_text('title_lower_top', $('#vs_text_bottom_upper').val())
	await window.kbmodules.futsal_advanced.titles.splash.set_text('title_lower_bot', $('#vs_text_bottom_lower').val())

	// Set main header
	await window.kbmodules.futsal_advanced.titles.splash.set_text('title_upper', $('#vs_text_main_header').val())

	// Set logos
	await window.kbmodules.futsal_advanced.titles.splash.set_img_src(
		'logo_l',
		window.kbmodules.futsal_advanced.resource_index?.home_club?.logo_path || ''
	)
	await window.kbmodules.futsal_advanced.titles.splash.set_img_src(
		'logo_r',
		window.kbmodules.futsal_advanced.resource_index?.guest_club?.logo_path || ''
	)

	// Set team colours
	await window.kbmodules.futsal_advanced.titles.splash.set_shape_color(
		'team_color_l',
		window.kbmodules.futsal_advanced.resource_index.side.home?.lineup?.tshirt_colpick?.color
	)
	await window.kbmodules.futsal_advanced.titles.splash.set_shape_color(
	...
	// Set Club names
	await window.kbmodules.futsal_advanced.titles.splash.set_text(
		'club_name_l',
		ksys.strf.params.club_name.format(
			window.kbmodules.futsal_advanced.resource_index?.home_club?.club_name || ''
		)
	)
	await window.kbmodules.futsal_advanced.titles.splash.set_text(
		'club_name_r',
		ksys.strf.params.club_name.format(
			window.kbmodules.futsal_advanced.resource_index?.guest_club?.club_name || ''
		)
	)

	// Show the title
	await window.kbmodules.futsal_advanced.titles.splash.overlay_in()

	// unlock the button
	ksys.btns.pool.show_splash.toggle(true)
}

window.kbmodules.futsal_advanced.hide_vs_title = async function(){
	ksys.btns.pool.show_splash.toggle(false)
	await window.kbmodules.futsal_advanced.titles.splash.overlay_out()
	ksys.btns.pool.show_splash.toggle(true)
}
...
//        Coach stuff
// ================================
window.kbmodules.futsal_advanced.show_coach = async function(side){

	const tgt_club = window.kbmodules.futsal_advanced.resource_index.side?.[str(side).lower()]?.club;
	...
	await window.kbmodules.futsal_advanced.titles.coach.set_text(
		'name',
	...
	await window.kbmodules.futsal_advanced.titles.coach.set_img_src(
		'club_logo',
		tgt_club.logo_path
	)

	ksys.btns.adv_timeout({
		'show_coach_home_team': window.kbmodules.futsal_advanced.titles.coach.anim_durations[null],
		'show_coach_guest_team': window.kbmodules.futsal_advanced.titles.coach.anim_durations[null],

		'hide_coach_home_team': window.kbmodules.futsal_advanced.titles.coach.anim_durations[null],
		'hide_coach_guest_team': window.kbmodules.futsal_advanced.titles.coach.anim_durations[null],
	})

	await window.kbmodules.futsal_advanced.titles.coach.overlay_in()
...
window.kbmodules.futsal_advanced.hide_coach = async function(){
	ksys.btns.adv_timeout({
		'show_coach_home_team': window.kbmodules.futsal_advanced.titles.coach.anim_durations['TransitionOut'],
		'show_coach_guest_team': window.kbmodules.futsal_advanced.titles.coach.anim_durations['TransitionOut'],

		'hide_coach_home_team': window.kbmodules.futsal_advanced.titles.coach.anim_durations['TransitionOut'],
		'hide_coach_guest_team': window.kbmodules.futsal_advanced.titles.coach.anim_durations['TransitionOut'],
	})

	await window.kbmodules.futsal_advanced.titles.coach.overlay_out()
}
...
//        Card stuff
// ================================
window.kbmodules.futsal_advanced.hand_card = async function(card_type){
	// sanity check
	...
	const sel_entry = window.kbmodules.futsal_advanced.resource_index.card_player_filter.selected_entry;
	const card_manager = window.kbmodules.futsal_advanced.resource_index.card_manager;
	if (!sel_entry){
		ksys.info_msg.send_msg(
			`No player selected`,
...
// todo: duplicated code
window.kbmodules.futsal_advanced.pardon_player = async function(){
	// ensure there's a player selected
	const sel_entry = window.kbmodules.futsal_advanced.resource_index.card_player_filter.selected_entry;
	const card_manager = window.kbmodules.futsal_advanced.resource_index.card_manager;
	if (!sel_entry){
...
window.kbmodules.futsal_advanced.hide_card = async function(){
...
//        Substitute
// ================================
window.kbmodules.futsal_advanced.substitute_player_pair = function(){
	// todo: is this stupid ?
	const leaving_player = (
		window.kbmodules.futsal_advanced.resource_index.side.home.substitute['leaving']?.selected_entry?.player
		||
		window.kbmodules.futsal_advanced.resource_index.side.guest.substitute['leaving']?.selected_entry?.player
	);
	const incoming_player = (
		window.kbmodules.futsal_advanced.resource_index.side.home.substitute['inbound']?.selected_entry?.player
		||
		window.kbmodules.futsal_advanced.resource_index.side.guest.substitute['inbound']?.selected_entry?.player
	);
...
// SINGLE
window.kbmodules.futsal_advanced.exec_substitute = async function(subs_type){
	const [leaving_player, incoming_player] = window.kbmodules.futsal_advanced.substitute_player_pair();
	if (!leaving_player || !incoming_player){
		return
	}
	...
	if (leaving_player.club.is_enemy){
		stack = window.kbmodules.futsal_advanced.resource_index.guest_lineup.subs;
	}else{
		stack = window.kbmodules.futsal_advanced.resource_index.home_lineup.subs;
	}
...
window.kbmodules.futsal_advanced.stack_substitute = function(){
	const [leaving_player, incoming_player] = window.kbmodules.futsal_advanced.substitute_player_pair();
	if (!leaving_player || !incoming_player){
		return
	}
...
window.kbmodules.futsal_advanced.toggle_config_vis = function(){
...
window.kbmodules.futsal_advanced.apply_preset = function(preset_id, evt=null){
	if (evt && !evt.altKey){
		ksys.info_msg.send_msg(
			`Hold ALT`,
			'warn',
...
window.kbmodules.futsal_advanced.set_weather_icon = async function(tgt_icon){
...
window.kbmodules.futsal_advanced.timeout_vis = async function(state){
	if (state == true){
		ksys.btns.adv_timeout({
			'show_timeout_title': window.kbmodules.futsal_advanced.titles.timeout.anim_durations[null],
			'hide_timeout_title': window.kbmodules.futsal_advanced.titles.timeout.anim_durations[null],
		})
		await window.kbmodules.futsal_advanced.titles.timeout.overlay_in();
	}
	if (state == false){
		ksys.btns.adv_timeout({
			'hide_timeout_title': window.kbmodules.futsal_advanced.titles.timeout.anim_durations['TransitionOut'],
			'show_timeout_title': window.kbmodules.futsal_advanced.titles.timeout.anim_durations['TransitionOut'],
		})
		await window.kbmodules.futsal_advanced.titles.timeout.overlay_out();
	}
}
...
//      Shared
// ----------------
window.kbmodules.futsal_advanced.update_extra_time_amount = async function(){
	const amount = $('#timer_ctrl_additional input').val().trim();
	await window.kbmodules.futsal_advanced.titles.timer.set_text(
		'time_added',
		amount ? `+${amount}` : '',
	)
}
...
window.kbmodules.futsal_advanced.update_selected_timer_system = async function(_, tgt_sys, event){
	if (!tgt_sys){
		ksys.info_msg.send_msg(
	...
	ksys.context.module.prm('timer_sys', window.kbmodules.futsal_advanced.resource_index.timer_sys_switch.selected);

	// First, kill AT-AT watcher
	await window.kbmodules.futsal_advanced.stop_atat_service();

	// Then, kill all timers

	// AT-AT Base
	await window.kbmodules.futsal_advanced.timer_fset.at_at?.base_counter?.terminate?.();
	// await window.kbmodules.futsal_advanced.timer_fset.at_at.base_counter.terminate();
	window.kbmodules.futsal_advanced.timer_fset.at_at.base_counter = null;
	// AT-AT Extra
	await window.kbmodules.futsal_advanced.timer_fset.at_at?.extra_counter?.terminate?.();
	window.kbmodules.futsal_advanced.timer_fset.at_at.extra_counter = null;

	// Builtin Base
	window.kbmodules.futsal_advanced?.base_counter?.force_kill?.();
	window.kbmodules.futsal_advanced.base_counter = null;
	// Builtin Extra
	window.kbmodules.futsal_advanced?.extra_counter?.force_kill?.();
	window.kbmodules.futsal_advanced.extra_counter = null;

	// Restart AT-AT system
	// await ksys.ticker.kb_at.sys_restart();

	// Set feature set reference
	window.kbmodules.futsal_advanced.timer_ctrl = window.kbmodules.futsal_advanced.timer_fset[tgt_sys];
	...
	if (tgt_sys == 'at_at'){
		...
		// if (window.kbmodules.futsal_advanced.timer_fset.at_at.base_counter){
	...
	if (tgt_sys == 'builtin'){
		...
		window.kbmodules.futsal_advanced.stop_atat_service();
...
window.kbmodules.futsal_advanced._get_current_time = function(minutes=false, tsum=false){
	...
	if (tsum){
		...
		Math.ceil(
			(
				(window.kbmodules.futsal_advanced?.base_counter?.tick?.global || 1)
				+
				(window.kbmodules.futsal_advanced?.extra_counter?.tick?.global || 1)
			)
			/
			divider
		)
		...
		let extra_t = window.kbmodules.futsal_advanced?.extra_counter?.tick?.global;
		if (window.kbmodules.futsal_advanced?.extra_counter?.tick?.global === 0) {
			extra_t = 1;
		}

		let base_t = Math.ceil(
			(window.kbmodules.futsal_advanced?.base_counter?.tick?.global || 1) / divider
		);
...
window.kbmodules.futsal_advanced.get_current_time = function(minutes=false, tsum=false){
	const divider = (minutes ? 60 : 1);
	const dur = window.kbmodules.futsal_advanced.round_duration;
			...
			(
				(
					(
						...
						(window.kbmodules.futsal_advanced.ticker_time.base.seconds)
					)
					|| 0
				)
				+
				(
					(
						(window.kbmodules.futsal_advanced.ticker_time.extra.minutes * 60) +
...
window.kbmodules.futsal_advanced.main_timer_vis = async function(state){
	const title = window.kbmodules.futsal_advanced.titles.timer;

	// await window.kbmodules.futsal_advanced.update_team_colors();
	...
	if (state == true){
		...
		if ( (window.kbmodules.futsal_advanced.ticker_time.extra.minutes <= 0) && (window.kbmodules.futsal_advanced.ticker_time.extra.seconds <= 0) ){
			await window.kbmodules.futsal_advanced.extra_time_vis(false);
		}

		// Set period text
		// const current_total = (window.kbmodules.futsal_advanced.ticker_time.base.minutes * 60) + window.kbmodules.futsal_advanced.ticker_time.base.seconds;
		// const dur_total = window.kbmodules.futsal_advanced.round_duration * 60;
		// await title.set_text(
		// 	'period',
		// 	// window.kbmodules.futsal_advanced.ticker_time.base.minutes window.kbmodules.futsal_advanced.round_duration
		// 	(current_total <= dur_total) ? '1 ST' : '2 ND'
		// )

		// Redraw fouls
		await window.kbmodules.futsal_advanced.resource_index.side.guest?.lineup?.fouls?.redrawVMIX?.();
		await window.kbmodules.futsal_advanced.resource_index.side.home?.lineup?.fouls?.redrawVMIX?.();

		// todo: hardcoded pages
		await window.kbmodules.futsal_advanced.titles.timer.page(2);

		await window.kbmodules.futsal_advanced.resource_index.card_manager.redraw_counters();
		...
		await title.set_text(
			'command_l',
			str(window.kbmodules.futsal_advanced.resource_index.side.home.club.club_name_shorthand).upper()
		)
		await title.set_text(
			'command_r',
			str(window.kbmodules.futsal_advanced.resource_index.side.guest.club.club_name_shorthand).upper()
		)
...
window.kbmodules.futsal_advanced.extra_time_vis = async function(state){
	// await window.kbmodules.futsal_advanced.titles.timer.resume_render();
	// await window.kbmodules.futsal_advanced.titles.timer.pause_render();
	if (state == true){
		// await window.kbmodules.futsal_advanced.titles.timer.set_shape_color('main_time_bg', 'D80D8300');
		// await window.kbmodules.futsal_advanced.titles.timer.toggle_text('time_added', true);
		// await window.kbmodules.futsal_advanced.titles.timer.toggle_text('extra_ticker', true);
		// await window.kbmodules.futsal_advanced.titles.timer.toggle_img('extra_time_bg', true);

		if (window.kbmodules.futsal_advanced.timer_main_clock_vis_schema_switch.selected == 'extra_only'){
			await window.kbmodules.futsal_advanced.titles.timer.toggle_img('main_time_bg', false);
			await window.kbmodules.futsal_advanced.titles.timer.toggle_text('base_ticker', false);
		}
	...
	if (state == false){
		await window.kbmodules.futsal_advanced.titles.timer.toggle_text('base_ticker', true);
		await window.kbmodules.futsal_advanced.titles.timer.toggle_img('main_time_bg', true);

		if (window.kbmodules.futsal_advanced.title_schema_switch.selected == 'paged'){
			await window.kbmodules.futsal_advanced.titles.timer.set_shape_color('main_time_bg', 'D80D83FF');
		}

		await window.kbmodules.futsal_advanced.titles.timer.toggle_text('time_added', false);
		await window.kbmodules.futsal_advanced.titles.timer.toggle_text('extra_ticker', false);
		await window.kbmodules.futsal_advanced.titles.timer.toggle_img('extra_time_bg', false);
	}
	// await window.kbmodules.futsal_advanced.titles.timer.resume_render();
}

window.kbmodules.futsal_advanced.zero_timer_time = function(){
	window.kbmodules.futsal_advanced.ticker_time.base.minutes = 0;
	window.kbmodules.futsal_advanced.ticker_time.base.seconds = 0;

	window.kbmodules.futsal_advanced.ticker_time.extra.minutes = 0;
	window.kbmodules.futsal_advanced.ticker_time.extra.seconds = 0;
}

window.kbmodules.futsal_advanced.show_red_cards = async function(){
	await window.kbmodules.futsal_advanced.titles.timer.page(2);
}
window.kbmodules.futsal_advanced.hide_red_cards = async function(){
	await window.kbmodules.futsal_advanced.titles.timer.page(1);
}
...
// 
window.kbmodules.futsal_advanced.update_atat_status = function(status){
	...
	if (status.ok){
		...
		if (!window.kbmodules.futsal_advanced.at_at.btns_enable_lock){
			ksys.btns.toggle({
				'launch_main_timer_r1': true,
				'launch_main_timer_r2': true,
				'continue_from_time':   true,
...
window.kbmodules.futsal_advanced.update_atat_port = function(evt){
	if (!evt.altKey){return};
...
window.kbmodules.futsal_advanced.update_atat_return_addr = function(evt){
	if (!evt.altKey){return};
...
window.kbmodules.futsal_advanced.restart_atat_service = async function(){
	...
	window.kbmodules.futsal_advanced.at_at.service_ping = ksys.ticker.kb_at.StatusWatcher(
		ksys.context.global.cache.vmix_ip,
		atat_port,
		window.kbmodules.futsal_advanced.update_atat_status
	)
}

window.kbmodules.futsal_advanced.stop_atat_service = async function(){
	await window.kbmodules.futsal_advanced.at_at?.service_ping?.terminate?.();
}

window.kbmodules.futsal_advanced.toggle_atat_status_indicator = function(state){
	if (state == true){
...
// Util
// 
window.kbmodules.futsal_advanced.timeout_atat_btns = async function(timeout_ms=1000){
	window.kbmodules.futsal_advanced.at_at.btns_enable_lock = true;
	ksys.btns.toggle({
		'launch_main_timer_r1': false,
		'launch_main_timer_r2': false,
		'continue_from_time':   false,
	...
	window.kbmodules.futsal_advanced.at_at.btns_enable_lock = false;
	ksys.btns.toggle({
		'launch_main_timer_r1': true,
		'launch_main_timer_r2': true,
		'continue_from_time':   true,
...
window.kbmodules.futsal_advanced.toggle_atat_btns = function(state=true){
	if (state == true){
		window.kbmodules.futsal_advanced.at_at.btns_enable_lock = false;
		ksys.btns.toggle({
			'launch_main_timer_r1': true,
...
window.kbmodules.futsal_advanced.restore_atat_line = async function(){
	// 
	// Main timer
	// 
	window.kbmodules.futsal_advanced.timer_fset.at_at.create_base_timer();
	const base_timer_time = await window.kbmodules.futsal_advanced.timer_fset.at_at.base_counter.get_curtime();
	...
	await window.kbmodules.futsal_advanced.timer_fset.at_at.base_counter.resub_to_echo();
	await window.kbmodules.futsal_advanced.timer_fset.at_at.base_counter.resub_to_end();

	// 
	// Extra timer
	// 
	window.kbmodules.futsal_advanced.timer_fset.at_at.create_extra_time()
	const extra_timer_time = await window.kbmodules.futsal_advanced.timer_fset.at_at.extra_counter.get_curtime();
	console.error(extra_timer_time);

	if (!extra_timer_time.ok || !extra_timer_time.reply_data){return};
	// If timer exists and well - reattach everything to it
	await window.kbmodules.futsal_advanced.timer_fset.at_at.extra_counter.resub_to_echo();
	await window.kbmodules.futsal_advanced.timer_fset.at_at.extra_counter.resub_to_end();
}

window.kbmodules.futsal_advanced.atat_auto_return_addr = async function(){
...
// Base Timer
// 
window.kbmodules.futsal_advanced.timer_fset.at_at.create_base_timer = function(rnum){
	const dur = window.kbmodules.futsal_advanced.round_duration;

	window.kbmodules.futsal_advanced.timer_fset.at_at.base_counter = new ksys.ticker.kb_at.AtAtTicker({
		'id':   2,
		'ip':   ksys.context.global.cache.vmix_ip,
		'port': ksys.context.global.cache.atat_port,
		// todo: Better url construction
		'url_params': `/API/?Function=SetText&Input=${window.kbmodules.futsal_advanced.titles.timer.title_name}&SelectedName=base_ticker.Text&Value=\0`,
		'timings': {
			'start': [
				(rnum == 1) ? 0 : dur,
				0,
		...
		'end_callback': async function(){
			...
			if (ksys.context.module.cache.round_num){
				await window.kbmodules.futsal_advanced.timer_fset.at_at.launch_extra_time();
				window.kbmodules.futsal_advanced.extra_time_vis(true);
			}
		},
		'echo_callback': window.kbmodules.futsal_advanced.timer_fset.at_at.timer_callback,
	})
}

window.kbmodules.futsal_advanced.timer_fset.at_at.wipe_timers = async function(rnum=1){
	// Kill base timer
	await window.kbmodules.futsal_advanced.timer_fset.at_at?.base_counter?.terminate?.();
	window.kbmodules.futsal_advanced.timer_fset.at_at.base_counter = null;
	...
	window.kbmodules.futsal_advanced.zero_timer_time();

	// todo: It's speculated, that each time a new main timer starts - 
	// the extra time should be cleared.
	await window.kbmodules.futsal_advanced.titles.timer.set_text('time_added', '');
	await window.kbmodules.futsal_advanced.titles.timer.set_text('extra_ticker', '00:00');
	await window.kbmodules.futsal_advanced.extra_time_vis(false);

	// Clear the base ticker text field
	await window.kbmodules.futsal_advanced.titles.timer.set_text(
		'base_ticker', (rnum == 1) ? '00:00' : `${window.kbmodules.futsal_advanced.round_duration}:00`
	);
}

window.kbmodules.futsal_advanced.timer_fset.at_at.start_base_timer = async function(rnum){
	window.kbmodules.futsal_advanced.toggle_atat_btns(false);
	window.kbmodules.futsal_advanced.ticker_time.extra.minutes = 0;
	window.kbmodules.futsal_advanced.ticker_time.extra.seconds = 0;
	...
	// todo: shouldn't timer controls be stored in window.kbmodules.futsal_advanced.at_at ?

	// todo: It's speculated, that each time a new main timer starts - 
	// the extra time should be cleared.
	// The function below does wipe the extra timer.
	await window.kbmodules.futsal_advanced.timer_fset.at_at.wipe_timers(rnum);
	$('#timer_ctrl_additional input')[0].value = '';


	// Instantiate AT-AT ticker class
	window.kbmodules.futsal_advanced.timer_fset.at_at.create_base_timer(rnum);

	// Launch the AT-AT ticker
	print(
		'Start response:',
		await window.kbmodules.futsal_advanced.timer_fset.at_at.base_counter.start()
	);

	window.kbmodules.futsal_advanced.timeout_atat_btns();
}

window.kbmodules.futsal_advanced.timer_fset.at_at.timer_callback = function(msg){
	if (msg.data_buf[0] != 2){
		console.warn(
	...
	window.kbmodules.futsal_advanced.ticker_time.base.minutes = msg.data_buf[2];
	window.kbmodules.futsal_advanced.ticker_time.base.seconds = msg.data_buf[3];
	...
	// window.kbmodules.futsal_advanced.titles.timer.set_text('base_ticker', text);
	$('#timer_feedback_main').text(text);
}

window.kbmodules.futsal_advanced.timer_fset.at_at.resume_main_timer_from_offset = async function(){
	if (!window.kbmodules.futsal_advanced.timer_fset.at_at.base_counter){
	...
	await window.kbmodules.futsal_advanced.timer_fset.at_at.base_counter.resume_from_offset({
...
// Extra Timer
// 
window.kbmodules.futsal_advanced.timer_fset.at_at.create_extra_time = function(){
	window.kbmodules.futsal_advanced.timer_fset.at_at.extra_counter = new ksys.ticker.kb_at.AtAtTicker({
		'id':   3,
		'ip':   ksys.context.global.cache.vmix_ip,
		'port': ksys.context.global.cache.atat_port,
		// todo: Better url construction
		'url_params': `/API/?Function=SetText&Input=${window.kbmodules.futsal_advanced.titles.timer.title_name}&SelectedName=extra_ticker.Text&Value=\0`,
		'timings': {
			'start': [0, 0],
			'end': [60, 0],
		},
		'end_callback': null,
		'echo_callback': window.kbmodules.futsal_advanced.timer_fset.at_at.extra_timer_callback,
	})
}

window.kbmodules.futsal_advanced.timer_fset.at_at.launch_extra_time = async function(){
	// Todo: kill previous timers or simply overwrite with new data?
	// Commit murder for now
	...
	print(
		...
		await window.kbmodules.futsal_advanced.timer_fset.at_at.extra_counter.start()
	);
}

window.kbmodules.futsal_advanced.timer_fset.at_at.extra_timer_callback = function(msg){
	...
	window.kbmodules.futsal_advanced.ticker_time.extra.minutes = msg.data_buf[2];
	window.kbmodules.futsal_advanced.ticker_time.extra.seconds = msg.data_buf[3];

	const text = `${str(msg.data_buf[2]).zfill(2)}:${str(msg.data_buf[3]).zfill(2)}`;
	// const text = `${msg.data_buf[2]}:${msg.data_buf[3]}`;

	// window.kbmodules.futsal_advanced.titles.timer.set_text('extra_ticker', text);
	$('#timer_feedback_extra').text(text);
}

window.kbmodules.futsal_advanced.timer_fset.at_at.stop_extra_time = async function(){
	window.kbmodules.futsal_advanced.timeout_atat_btns();
	await window.kbmodules.futsal_advanced.timer_fset.at_at?.extra_counter?.terminate?.();
	window.kbmodules.futsal_advanced.timer_fset.at_at.extra_counter = null;
}
...
window.kbmodules.futsal_advanced.timer_fset.builtin._start_base_timer = async function(rnum){
	...
	window.kbmodules.futsal_advanced.titles.timer.set_text('time_added', '')
	$('#timer_ctrl_additional input')[0].value = '';
	...
	await window.kbmodules.futsal_advanced.titles.timer.set_text('base_ticker', (rnum == 1) ? '00:00' : `${dur}:00`);

	window.kbmodules.futsal_advanced.base_counter = ksys.ticker.spawn({
		...
		'infinite': false,
		'reversed': false,
		'callback': window.kbmodules.futsal_advanced.timer_fset.builtin.timer_callback,
		'wait': true,
	})

	window.kbmodules.futsal_advanced.base_counter.fire()
	.then(function(_ticker) {
		...
		if (_ticker){
			...
			// if (document.querySelector('#timer_ctrl_additional input').value.trim()){
				window.kbmodules.futsal_advanced.timer_fset.builtin.launch_extra_time()
			}
			*/
			if (!pre_killed){
				window.kbmodules.futsal_advanced.timer_fset.builtin.launch_extra_time()
	...
	// print(window.kbmodules.futsal_advanced.base_counter)
}

window.kbmodules.futsal_advanced.timer_fset.builtin.start_base_timer = async function(rnum){
	...
	// Reset fouls
	window.kbmodules.futsal_advanced.resource_index?.home_lineup?.fouls?.setFouls?.(0);
	window.kbmodules.futsal_advanced.resource_index?.guest_lineup?.fouls?.setFouls?.(0);
	await window.kbmodules.futsal_advanced.resource_index?.home_lineup?.fouls?.redrawVMIX?.();
	await window.kbmodules.futsal_advanced.resource_index?.guest_lineup?.fouls?.redrawVMIX?.();

	// Destroy any extra time things
	await window.kbmodules.futsal_advanced.timer_fset.builtin.destroy_extra_clock();
	...
	// Get round duration
	const dur = window.kbmodules.futsal_advanced.round_duration * 60;
		...
		'data': {
			'clock': {
				...
				'vmix_fields': [{
					'count_as': 'clock',
					'tplate': '%mt%:%s%',
					'gtzip_name': window.kbmodules.futsal_advanced.titles.timer.title_name,
...
window.kbmodules.futsal_advanced.timer_fset.builtin.base_timer_end_callback = async function(data){
	print('BASE TIMER END CALLBACK')
	// simply start extra time
	// window.kbmodules.futsal_advanced.timer_fset.builtin.launch_extra_time();
}

window.kbmodules.futsal_advanced.timer_fset.builtin.destroy_base_timer = async function(){
	await ksys.ticker.bundestag.edit_clock({
		'clock_id': window.kbmodules.futsal_advanced.main_clock_id,
		'action': 'stop',
		'data': {}
...
window.kbmodules.futsal_advanced.timer_fset.builtin._timer_callback = function(tick){
	const minutes = Math.floor(tick.global / 60);
	const seconds = tick.global - (60*minutes);

	window.kbmodules.futsal_advanced.ticker_time.base.minutes = minutes;
	window.kbmodules.futsal_advanced.ticker_time.base.seconds = seconds;
	...
	if (text == `${dur}:01`){
		text = `${dur}:00`;
		window.kbmodules.futsal_advanced.ticker_time.base.minutes = dur;
		window.kbmodules.futsal_advanced.ticker_time.base.seconds = 0;
	}

	if (text == '90:01'){
		text = '90:00';
		window.kbmodules.futsal_advanced.ticker_time.base.minutes = dur * 2;
		window.kbmodules.futsal_advanced.ticker_time.base.seconds = 0;
	}
...
window.kbmodules.futsal_advanced.timer_fset.builtin.timer_callback = function(data){
	if (!data?.time?.total){return};
	const time = data.time;

	window.kbmodules.futsal_advanced.ticker_time.base.minutes = time.total.minutes;
	window.kbmodules.futsal_advanced.ticker_time.base.seconds = time.clock.seconds;
...
window.kbmodules.futsal_advanced.timer_fset.builtin._resume_main_timer_from_offset = function(event){
	...
	window.kbmodules.futsal_advanced.titles.timer.set_text('time_added', '');
	window.kbmodules.futsal_advanced.extra_time_vis(false);
...
window.kbmodules.futsal_advanced.timer_fset.builtin.resume_main_timer_from_offset = async function(event){
	// Destroy any extra time things
	await window.kbmodules.futsal_advanced.timer_fset.builtin.destroy_extra_clock();
...
window.kbmodules.futsal_advanced.timer_fset.builtin.pause = async function(){
	await ksys.ticker.bundestag.edit_clock({
		'clock_id': window.kbmodules.futsal_advanced.main_clock_id,
		'action': 'pause',
		'data': {}
...
window.kbmodules.futsal_advanced.timer_fset.builtin.resume = async function(){
	await ksys.ticker.bundestag.edit_clock({
		'clock_id': window.kbmodules.futsal_advanced.main_clock_id,
		'action': 'resume',
		'data': {}
...
window.kbmodules.futsal_advanced._edit_main_clock_offset = function(offs){
	const elem = qsel('#base_timer_resume_input .seconds');
	elem.value = int(elem.value) + offs;
}

window.kbmodules.futsal_advanced.edit_main_clock_offset = function(offs){
...
window.kbmodules.futsal_advanced.timer_fset.builtin.destroy_extra_clock = async function(){
	...
	await ksys.ticker.bundestag.reset_clock(window.kbmodules.futsal_advanced.extra_clock_id);
...
window.kbmodules.futsal_advanced.timer_fset.builtin._launch_extra_time = async function(){
	window.kbmodules.futsal_advanced?.extra_counter?.force_kill?.()
	window.kbmodules.futsal_advanced.extra_counter = null;

	await window.kbmodules.futsal_advanced.update_extra_time_amount()
	...
	*/
	...
	window.kbmodules.futsal_advanced.extra_counter = ksys.ticker.spawn({
		'duration': extra_amount*60,
		'name': `gigas_timer${1}`,
		'infinite': true,
		'reversed': false,
		'callback': window.kbmodules.futsal_advanced.timer_fset.builtin.extra_timer_callback,
		'wait': true,
	})

	window.kbmodules.futsal_advanced.extra_counter.fire()
	.then(function(_ticker) {
		// turn off automatically
		if (_ticker){
			_ticker.force_kill()
	...
	await window.kbmodules.futsal_advanced.titles.timer.set_text('extra_ticker', '00:00');
	// await window.kbmodules.futsal_advanced.titles.timer.set_text('time_added', `+${Math.floor(extra_amount/1)}`)
	// await window.kbmodules.futsal_advanced.titles.timer.toggle_text('time_added', true)
	// await window.kbmodules.futsal_advanced.titles.timer.toggle_img('extra_time_bg', true)
	// await window.kbmodules.futsal_advanced.titles.timer.toggle_text('extra_ticker', true)
}

window.kbmodules.futsal_advanced.timer_fset.builtin.launch_extra_time = async function(){
	// Stop extra time clock
	await ksys.ticker.bundestag.edit_clock({
		'clock_id': window.kbmodules.futsal_advanced.extra_clock_id,
		'action': 'stop',
	...
	const fuck_shit = window.kbmodules.futsal_advanced.timer_display_schema_switch.selected == 'fuck_shit';
	...
	await ksys.ticker.bundestag.edit_clock({
		...
		'data': {
			'clock': {
				...
				'duration': 60**2,
				'reversed': false,
				'fuck_you': fuck_shit ? (window.kbmodules.futsal_advanced.ticker_time.base.minutes * 60) : 0,
				'vmix_fields': [{
					'count_as': 'clock',
					'tplate': '%m%:%s%',
					'gtzip_name': window.kbmodules.futsal_advanced.titles.timer.title_name,
...
window.kbmodules.futsal_advanced.timer_fset.builtin.extra_time_end_callback = async function(){
	// todo: not much needs to be done here really.
	// or is it ?
}

window.kbmodules.futsal_advanced.timer_fset.builtin._extra_timer_callback = function(tick){
	const minutes = Math.floor(tick.global / 60)
	const seconds = tick.global - (60*minutes)

	window.kbmodules.futsal_advanced.ticker_time.extra.minutes = minutes;
	window.kbmodules.futsal_advanced.ticker_time.extra.seconds = seconds;
...
window.kbmodules.futsal_advanced.timer_fset.builtin.extra_timer_callback = function(data){
	if (!data?.time?.total){return};

	const time = data.time;

	window.kbmodules.futsal_advanced.ticker_time.extra.minutes = time.clock.minutes;
	window.kbmodules.futsal_advanced.ticker_time.extra.seconds = time.clock.seconds;
...
window.kbmodules.futsal_advanced.timer_fset.builtin.stop_extra_time = async function(){
	// Stop extra time clock
	await window.kbmodules.futsal_advanced.timer_fset.builtin.destroy_extra_clock();
}
...
window.kbmodules.futsal_advanced.add_score_from_cards_panel = async function(auto=false){
	const player = window.kbmodules.futsal_advanced.resource_index.card_player_filter?.selected_entry?.player;
	...
	// Show the title
	if (window.kbmodules.futsal_advanced.title_schema_switch.selected == 'separate'){
		// Set logo
		await window.kbmodules.futsal_advanced.titles.gscore.set_img_src(
			'club_logo',
			player.club.logo_path
		)

		// Set player's surname
		await window.kbmodules.futsal_advanced.titles.gscore.set_text(
			'player_name',
		...
		ksys.btns.adv_timeout({
			'scored': window.kbmodules.futsal_advanced.sequencer.sequences.scored.sum,
		})

		// Hide red cards
		await window.kbmodules.futsal_advanced.hide_red_cards();

		await window.kbmodules.futsal_advanced.titles.gscore.overlay_in()
		await ksys.util.sleep(
			window.kbmodules.futsal_advanced.sequencer.sequences.scored.items.hold.dur
		)
		await window.kbmodules.futsal_advanced.titles.gscore.overlay_out()
		await window.kbmodules.futsal_advanced.show_red_cards();
		...
		window.kbmodules.futsal_advanced.timeout_persistent_header_btns(
			window.kbmodules.futsal_advanced.sequencer.sequences.scored.sum
		);
		...
		await ksys.util.sleep(window.kbmodules.futsal_advanced.sequencer.sequences.scored.items.hold.dur);
...
window.kbmodules.futsal_advanced.hide_scored_title = async function(){
	await window.kbmodules.futsal_advanced.titles.gscore.overlay_out();
	// todo: Check if it's already on the required page
	await window.kbmodules.futsal_advanced.show_red_cards();
}

window.kbmodules.futsal_advanced.mod_score_author = function(evt){
	if (!evt.altKey){return};

	const selected_player = window.kbmodules.futsal_advanced.resource_index.score_manager.player_picker?.selected_entry?.player;
	const selected_score_entry = window.kbmodules.futsal_advanced.resource_index.score_manager.selected_score_record;
	if (!selected_player){
		ksys.info_msg.send_msg(
			`No player selected`,
			'warn',
	...
	window.kbmodules.futsal_advanced.resource_index.score_manager.modify_score_author()
...
// todo: get rid of ?.
window.kbmodules.futsal_advanced.show_score_summary = async function(){
	await window.kbmodules.futsal_advanced.update_team_colors();
	...
	// 
	for (const side of ['home', 'guest']){
		const score_stack = window.kbmodules.futsal_advanced.resource_index?.score_manager?.sides[side]?.score_list?.score_stack;
	...
	// ------------------------------
	const score_amt_l = window.kbmodules.futsal_advanced.resource_index?.score_manager?.sides?.home?.score_list?.score_stack?.size || 0;
	...
	// ------------------------------
	await window.kbmodules.futsal_advanced.titles.final_scores.toggle_img('anim_full', false)
	await window.kbmodules.futsal_advanced.titles.final_scores.toggle_img('anim_half', false)
	...
	window.kbmodules.futsal_advanced.titles.final_scores.set_img_src(
		'upper_bg',
		str(scores_bg_image)
	)
	...
	await window.kbmodules.futsal_advanced.titles.final_scores.set_text(
		'team_name_l',
		ksys.strf.params.club_name.format(window.kbmodules.futsal_advanced.resource_index.side.home?.club?.club_name)
	)
	// Club description LEFT
	await window.kbmodules.futsal_advanced.titles.final_scores.set_text(
		'descr_l',
		ksys.strf.params.club_name.format(window.kbmodules.futsal_advanced.resource_index.side.home?.club?.descr)
	)
	// team logo LEFT
	await window.kbmodules.futsal_advanced.titles.final_scores.set_img_src(
		'team_logo_l',
		window.kbmodules.futsal_advanced.resource_index.side.home?.club?.logo_path || ''
	)
	// team colour LEFT
	...
	await window.kbmodules.futsal_advanced.titles.final_scores.set_text(
		'team_name_r',
		ksys.strf.params.club_name.format(window.kbmodules.futsal_advanced.resource_index.side.guest?.club?.club_name || '')
	)
	// Club description RIGHT
	await window.kbmodules.futsal_advanced.titles.final_scores.set_text(
		'descr_r',
		ksys.strf.params.club_name.format(window.kbmodules.futsal_advanced.resource_index.side.guest?.club?.descr)
	)
	// team logo RIGHT
	await window.kbmodules.futsal_advanced.titles.final_scores.set_img_src(
		'team_logo_r',
	...
	// Set bottom lines text
	await window.kbmodules.futsal_advanced.titles.final_scores.set_text('title_lower_top', $('#vs_text_bottom_upper').val())
	await window.kbmodules.futsal_advanced.titles.final_scores.set_text('title_lower_bot', $('#vs_text_bottom_lower').val())

	// Set main header
	await window.kbmodules.futsal_advanced.titles.final_scores.set_text('title_upper', $('#vs_text_main_header').val())


	// show the title
	await window.kbmodules.futsal_advanced.titles.final_scores.overlay_in()
}

window.kbmodules.futsal_advanced.hide_score_summary = async function(){
	await window.kbmodules.futsal_advanced.titles.final_scores.overlay_out()
}
...
window.kbmodules.futsal_advanced.save_match_stats = function(){
	const save = {};
	console.time('Saving stats')
	for (const stat_name in window.kbmodules.futsal_advanced.stats_unit_pool){
		const stat = window.kbmodules.futsal_advanced.stats_unit_pool[stat_name];
		save[stat_name] = {
...
// stat unit
window.kbmodules.futsal_advanced.StatUnit = class {
	...
	set_value(team, val){
		...
		// save stats to file
		window.kbmodules.futsal_advanced.save_match_stats()
	}
...
window.kbmodules.futsal_advanced.show_team_stats = async function(){

	ksys.btns.pool.show_team_stats.toggle(false)

	for (const stat_name in window.kbmodules.futsal_advanced.stats_unit_pool){
		window.kbmodules.futsal_advanced.stats_unit_pool[stat_name].push_to_vmix()
	}

	// Set scores
	const score_amt_l = window.kbmodules.futsal_advanced.resource_index.score_manager.sides.home.score_list.score_stack.size;
	const score_amt_r = window.kbmodules.futsal_advanced.resource_index.score_manager.sides.guest.score_list.score_stack.size;
	await window.kbmodules.futsal_advanced.titles.stats.set_text('scores', `${score_amt_l} : ${score_amt_r}`);

	// set logos
	await window.kbmodules.futsal_advanced.titles.stats.set_img_src(
		'team_logo_l',
		window.kbmodules.futsal_advanced.resource_index.side.home?.club?.logo_path || ''
	)
	await window.kbmodules.futsal_advanced.titles.stats.set_img_src(
	...
	await window.kbmodules.futsal_advanced.titles.stats.set_text('bottom_text', $('#vs_text_bottom_lower').val())
	...
	await window.kbmodules.futsal_advanced.titles.stats.overlay_in()

	ksys.btns.pool.show_team_stats.toggle(true)
}

window.kbmodules.futsal_advanced.hide_team_stats = async function(){

	ksys.btns.pool.hide_team_stats.toggle(false)

	await window.kbmodules.futsal_advanced.titles.stats.overlay_out()
...
*/
window.kbmodules.futsal_advanced.save_lineup_lists = function(){
	print('saving lineup lists');

	const save_data = {};

	const res_idx = window.kbmodules.futsal_advanced.resource_index;
...
window.kbmodules.futsal_advanced.load_lineup_lists = function(){
	...
	for (const side of ['home', 'guest']){
		...
		window.kbmodules.futsal_advanced.create_club_lineup(
			side,
			side_info.club_name,
			{
...
*/
window.kbmodules.futsal_advanced.save_field_layouts = function(){
	...
	for (const side of ['home', 'guest']){
		save_data[side].club_name = window.kbmodules.futsal_advanced.resource_index.side[side]?.club?.club_name?.lower?.();
		save_data[side].field = window.kbmodules.futsal_advanced.resource_index.side[side]?.field?.to_json?.() || {};
	}
...
window.kbmodules.futsal_advanced.load_field_layouts = function(){
	// Check if there's anything in the save file
	const last_save = ksys.db.module.read('field_layout.kbsave', 'json');
	if (!last_save){return};

	for (const side of ['home', 'guest']){
		if (!window.kbmodules.futsal_advanced.resource_index.side[side].field){continue};

		window.kbmodules.futsal_advanced.resource_index.side[side].field.apply_layout(last_save[side].field)
	}
}
...
//  - Player's name is changed in the club config panel
window.kbmodules.futsal_advanced.save_card_data = function(){
...
window.kbmodules.futsal_advanced.load_card_data = function(){
	// Check if there's anything in the save file
	const last_save = ksys.db.module.read('card_info.kbsave', 'json');
	if (!last_save){return};

	// sanity check
	if (!window.kbmodules.futsal_advanced.resource_index.card_manager){
		console.error(`Why doesn't the card manager exist ?????`);
		return
	}

	window.kbmodules.futsal_advanced.resource_index.card_manager.apply_data(last_save)
}
...
//  - Player's name is changed in the club config panel
window.kbmodules.futsal_advanced.save_score_data = function(){
	print('Saving score data')

	const score_manager = window.kbmodules.futsal_advanced.resource_index.score_manager;
	if (!score_manager){
		ksys.info_msg.send_msg(
...
window.kbmodules.futsal_advanced.load_score_data = function(){
	// Check if there's anything in the save file
...
//  Misc. Titles
// ---------------
window.kbmodules.futsal_advanced.save_misc_titles = function(){
	const misc_title_data = {};

	for (const misc_title of Object.values(window.kbmodules.futsal_advanced.misc_titles)){
...
window.kbmodules.futsal_advanced.global_save = function(save_targets=null){
	...
	if (what_to_save.lineup_lists || save_targets == null){
		window.kbmodules.futsal_advanced.save_lineup_lists()
	}

	// Save field layouts
	if (what_to_save.field_layout || save_targets == null){
		window.kbmodules.futsal_advanced.save_field_layouts()
	}

	// Save card data
	if (what_to_save.card_data || save_targets == null){
		window.kbmodules.futsal_advanced.save_card_data()
	}

	// Save score data
	if (what_to_save.scores || save_targets == null){
		window.kbmodules.futsal_advanced.save_score_data()
	}
	...
	if (what_to_save.misc_titles || save_targets == null){
		window.kbmodules.futsal_advanced.save_misc_titles();
	}
}
...
//    Penalties
// ---------------
window.kbmodules.futsal_advanced.save_penalties = function(){
	const penalty_manager = window.kbmodules.futsal_advanced.resource_index.penalty_manager;
...
window.kbmodules.futsal_advanced.show_penalty_title = async function(){
	const tgt_title = window.kbmodules.futsal_advanced.titles.penalties;

	await window.kbmodules.futsal_advanced.resource_index.penalty_manager.resync_vmix();

	await ksys.util.sleep(500);
	await tgt_title.overlay_in(2);
}

window.kbmodules.futsal_advanced.hide_penalty_title = async function(){
	const tgt_title = window.kbmodules.futsal_advanced.titles.penalties;
...
// (Wipe previous data)
window.kbmodules.futsal_advanced.init_new_match = function(evt){
	if (!evt.ctrlKey){return};
