...
window.kbmodules.kickboxing_standard.load = function()
{
	...
	for (let rnd of range(int(document.querySelector('rounds').getAttribute('amount')))){
		roundspool.append($(`<round round_index=${rnd+1} onclick="window.kbmodules.kickboxing_standard.set_round(${rnd+1}, true)">${rnd+1}</round>`)[0])
	}

	// load pairs
	window.kbmodules.kickboxing_standard.load_paris()

	window.kbmodules.kickboxing_standard.toggle_edit(false)
	...
	window.kbmodules.kickboxing_standard.label_index = [
		{
			'label':    'HEIGHT',
	...
	window.kbmodules.kickboxing_standard.personal_label_index = [
		{
			'label':    'HEIGHT',
			'param_sel': 'p_param[p_height] input',
			'suffix': 'CM'
...
{
	// oncontextmenu="window.kbmodules.kickboxing_standard.del_pair(this)"
	// onclick="window.kbmodules.kickboxing_standard.upd_vs_title(this.getAttribute("pair_index"))"
	...
	pair.index.root.oncontextmenu = function(_self){
		window.kbmodules.kickboxing_standard.del_pair(_self.target.closest('pair'))
	}
	pair.index.pairnum.onclick = function(_self){
		window.kbmodules.kickboxing_standard.upd_vs_title(_self.target.getAttribute('pair_index'))
	}
	...
	// enumarate pairs
	window.kbmodules.kickboxing_standard.enumerate_pairs()
}

window.kbmodules.kickboxing_standard.enumerate_pairs = function()
{
...
window.kbmodules.kickboxing_standard.del_pair = function(pr)
{
	if (pr && (window.kbmodules.kickboxing_standard.edit_mode_active == true)){
		pr.remove()
		window.kbmodules.kickboxing_standard.enumerate_pairs()
	}
}
...
window.kbmodules.kickboxing_standard.toggle_edit = function(state=null)
{
	// Sometimes I stagger even myself with my genius
	if (state == false || state == true){
		window.kbmodules.kickboxing_standard.edit_mode_active = !state
	}


	if (window.kbmodules.kickboxing_standard.edit_mode_active == false){
	...
	if (window.kbmodules.kickboxing_standard.edit_mode_active == true){
		window.kbmodules.kickboxing_standard.edit_mode_active = false;
...
window.kbmodules.kickboxing_standard.save_pairs = function(tofile=false)
{
	var collected = []
...
window.kbmodules.kickboxing_standard.load_paris = function(overwrite=null)
{
	// wipe existing
	...
	for (let pair of pairs_dict){
		document.querySelector('#pairs_pool').append(lizard.ehtml(`
			<pair oncontextmenu="window.kbmodules.kickboxing_standard.del_pair(this)">
				<pairnum click_contrast onclick="window.kbmodules.kickboxing_standard.upd_vs_title(this.getAttribute('pair_index'))"></pairnum>
				<players>
					<player right click_contrast noclick onclick="window.kbmodules.kickboxing_standard.upd_personal_title(this)">
						<display></display>
						<p_param p_name>
							<descr>Name</descr>
					...
					<player left click_contrast noclick onclick="window.kbmodules.kickboxing_standard.upd_personal_title(this)">
						<display></display>
						<p_param p_name>
							<descr>Name</descr>
...
window.kbmodules.kickboxing_standard.save_timer_duration = function()
{
...
window.kbmodules.kickboxing_standard.upd_vs_title = async function(p_index=null)
{
	// if (!pindex){return}
	// do NOT do this in edit mode!
	if (window.kbmodules.kickboxing_standard.edit_mode_active == true){return}
	...
	// set boxes
	for (const _box_idx in window.kbmodules.kickboxing_standard.label_index){
		const prm_info = window.kbmodules.kickboxing_standard.label_index[_box_idx];
	...
	// set names
	await window.kbmodules.kickboxing_standard.titles.lower.set_text(
		`player_l`,
		frmt.format(
			$(pair_elem).find(`player[left] p_param[p_name] input`).val().trim()
		),
...
window.kbmodules.kickboxing_standard.upd_personal_title = async function(player)
{
	if (window.kbmodules.kickboxing_standard.edit_mode_active == true){return};
	...
	for (const i in window.kbmodules.kickboxing_standard.personal_label_index){
		const box = window.kbmodules.kickboxing_standard.personal_label_index[i]
		const idx = int(i) + 1
...
window.kbmodules.kickboxing_standard.set_round = function(r, resetround=false)
{
...
window.kbmodules.kickboxing_standard.respawn_timer = async function(show=false, st=false)
{
	...
	*/
	window.kbmodules.kickboxing_standard.titles.timer.set_text('timer_text', `${minutes}:${str(seconds).zfill(2)}`)
...
window.kbmodules.kickboxing_standard.timer_set_time = function(tm=null)
{
	if (window.kbmodules.kickboxing_standard.counter && tm){
		// set time
		// window.kbmodules.kickboxing_standard.counter.set_global_tick(tm, true)
...
window.kbmodules.kickboxing_standard.vs_onn = function()
{
	vmix.talker.talk({
		'Function': 'OverlayInput1In',
		'Input': 'vs_main.gtzip',
...
window.kbmodules.kickboxing_standard.vs_off = function()
{
	vmix.talker.talk({
		'Function': 'OverlayInput1Out',
		'Input': 'vs_main.gtzip',
...
window.kbmodules.kickboxing_standard.player_onn = function()
{
	window.kbmodules.kickboxing_standard.titles.lower.overlay_in(1)
}

window.kbmodules.kickboxing_standard.player_off = function()
{
	window.kbmodules.kickboxing_standard.titles.lower.overlay_out(1)
}



window.kbmodules.kickboxing_standard.load_pairs_from_file = async function()
{
...
window.kbmodules.kickboxing_standard.save_pairs_to_file = function()
{
	lizard.textdl('boxing_pairs.preset', window.kbmodules.kickboxing_standard.save_pairs(true))
}



window.kbmodules.kickboxing_standard.set_category = async function()
{
...
window.kbmodules.kickboxing_standard.save_round_count = function()
{
